...
void makePlots(bool doMultipliers,bool doRatio, bool doBkgOnly, bool zoom, string location)
{
 // ROOT::Cintex::Cintex::Enable();
  ...
  mc_NC->SetLineColor(mycolors[7]);
  mc_NC_Bkg->SetLineColor(mycolors[7]);
  mc_NC_WrongSign->SetLineColor(mycolors[7]);
  ...
  if(doRatio){
    ...
    gc->SetYTitle("Ratio data/MnvGENIEv1");
    if(doBkgOnly && !zoom){
  ...
  if(doRatio){
    ...
    gc2->SetYTitle("Ratio data/MnvGENIEv1");
    if(doBkgOnly && !zoom){
