...
------"<<std::endl;
  ...
  hname= Form("%s/Hists_PhysicsBackgd_with_SYS_t%d_z%02d_Nu_v1_NuMAD.root", outdir.c_str(), targetID, targetZ);
     }
   else{
  hname= Form("%s/Hists_PhysicsBackgd_withiout_SYS_t%d_z%02d_Nu_v1_NuMAD.root", outdir.c_str(), targetID, targetZ);
     }
...
MnvH1D* TunePhysBackground( MnvH1D *histo, string SB, string outdir, int targetID, int targetZ, string playlist, string var, bool cv_only ){
    ...
    TFile *scaleFactor = new TFile( Form("%s/PhysBGScaleFactors_t%d_z%d_%s_%s.root", outdir.c_str(), targetID, temp_tgtZ, playlist.c_str(), getenv("NUKECC_TAG") ), "read" );
...
MnvH1D*  TunePlasticBackground( MnvH1D *histo, string region, string outdir, int targetID, int targetZ, string playlist, string var ){
    ...
    TFile *scaleFactor = new TFile( Form("%s/CHScaleFactors_t%d_z%d_%s_%s.root", outdir.c_str(), targetID, targetZ, playlist.c_str(), getenv("NUKECC_TAG") ), "read" );
    assert(scaleFactor);
