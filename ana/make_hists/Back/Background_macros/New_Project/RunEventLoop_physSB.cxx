...
int main(int argc, char *argv[]){
  ...
  //=========================================
   ...
   if(RunCodeWithSystematics){
       histFileName = utils->GetHistFileName( "EventSelection_phys_with_sys", FileType::kAny, targetID, targetZ, helicity ); 
     }
   else{
       histFileName = utils->GetHistFileName( "EventSelection_phys_without_sys", FileType::kAny, targetID, targetZ, helicity );
     } 
...
void FillVariable( PlotUtils::ChainWrapper* chain, HelicityType::t_HelicityType helicity, NukeCCUtilsNSF *utils , NukeCC_Cuts *cutter ,NukeCC_Binning  *binsDef ,std::vector<Var*>& variables,std::vector<Var2D*>& variables2d,bool isMC,int targetID, int targetZ, const string playlist, bool doDIS){
  ...
  std::vector<double> planeDNNbin;
   ...
   //variables = {planeDNN,emu,enu};//{enu,ehad}; 
   variables = {emu}; 
...
//	     if (v->GetName()=="Enu") 
	     reco5++;
             ...
             double trueEhad = universe->GetEhadTrueGeV();
             double trueEnu = universe->GetEnuTrueGeV();
             double trueEmu = universe->GetMuonETrueGeV();

             //cout<< trueEhad <<"\t"<< trueQ2 <<"\t"<< trueW <<endl;
             cout<< recoQ2 <<"\t"<< recoW <<endl;
...
//             const bool isTrueinTrans  = (trueW < 2.0 && ((cutter->passTrueCCQE(universe)) || (cutter->PassLowWCutTrue(universe)) || (cutter->PassLowQ2TransTrue(universe)) || (cutter->PasstransTrue(universe))));
//             const bool isTrueinContin = (trueW > 2.0 && trueQ2 < 1.0 && (cutter->PassLowQ2CutTrue(universe)));
//
//             const bool isTrueinTrans  =  ((cutter->passTrueCCQE(universe)) || (cutter->PassLowWCutTrue(universe)) || (cutter->PassLowQ2TransTrue(universe)) || (cutter->PasstransTrue(universe)));
//             const bool isTrueinContin =  (cutter->PassLowQ2CutTrue(universe));
...
/*             if(cutter->IsInTrueMaterial(universe,targetID,targetZ,false)){
               if( cutter->passTrueCCQE(universe)){
                    ...
                    reco0++;
                    v->m_selected_mc_lowQ2.univHist(universe)->Fill(v->GetRecoValueX(*universe), v->GetRecoValueY(*universe), wgt);}
	       else if( cutter->PassTrueTrans( universe )){
                    reco00++;
                    v->m_selected_mc_lowW.univHist(universe)->Fill(v->GetRecoValueX(*universe), v->GetRecoValueY(*universe), wgt);}
