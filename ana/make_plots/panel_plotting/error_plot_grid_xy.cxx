...
void makePlots(bool pt, string location, bool drawGroups, int areanorm)
{
  ...
  if(!drawGroups){
    ...
    for(int n=0;n<names.size();n++){
      ...
      if(pt){
	//GridCanvas* gcPT=plotXAxis1DRebinPz(histsPT2D, "P_{||} (GeV)", "P_{t}", 4,4,800,500,NULL);
	GridCanvas* gcPT=plotXAxis1DRebinPz(histsPT2D, "y", "x", 3,3,800,500,NULL);
	gcPT->SetYTitle("Fractional uncertainty");
	...
	gcPT->Print(Form("errors-%s-x_areanorm_%d.png",group.c_str(),areanorm));
	...
	gcPT->Print(Form("errors-%s-y_areanorm_%d.png",group.c_str(),areanorm));
