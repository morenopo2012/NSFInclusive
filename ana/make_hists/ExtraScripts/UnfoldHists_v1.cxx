...
bool UnfoldHisto( MnvH1D* hUnfolded, MnvH1D *hFolded, const std::string& var, int targetID, int targetZ, string playist, HelicityType::t_HelicityType helicity, bool accCorr /*=false*/);
...
------"<<std::endl;
  ...
  //vars.push_back("y");
   ...
   TFile *fUnfold = new TFile( Form("%s/CHScaleFactors_t%d_z%d_%s.root", outdir.c_str(), targetID, targetZ, playlist.c_str() ), "recreate" );
   assert(fUnfold);

//   TString fileName = utils->GetHistFileName( "EventSelection", FileType::kAny, targetID, targetZ, helicity );
   TFile *fin = new TFile( hname1, "RECREATE" );
  

//   TString BGfileName = utils->GetHistFileName( "EventSelection", FileType::kAny, targetID, targetZ, helicity );
   TFile *BGfin = new TFile( hname1, "RECREATE" );


  TParameter<double> *mcPOT = (TParameter<double>*)fin->Get("MCPOT");
  TParameter<double> *dataPOT = (TParameter<double>*)fin->Get("DataPOT");
  ...
  cout<<"Data POT = "<<datapot<<"   MC POT = "<<mcpot<<"   Data/MC POT = "<<dataMCScale<<endl;
   ...
   // {
       ...
       //TString hname = utils->GetHistName( "sample", FileType::kAny, var, targetID, targetZ );
        ...
        //Get the sample
        MnvH1D *mnv  = (MnvH1D*) f1->Get( Form("h_mc_%s",  var.c_str()));
        ...
        // Signal
        // Transition
        MnvH1D *mnv_true_trans_bg_target  = (MnvH1D*) f1->Get( Form("hists_trans_in_trans_%s",  var.c_str()));
        hists_trans_SB.push_back( mnv_true_trans_bg_target );
        // Continum 
        MnvH1D *mnv_true_contin_bg_target = (MnvH1D*) f1->Get( Form("hists_contin_in_trans_%s", var.c_str()));
    ...
    for( unsigned int ivar = 0; ivar != vars.size(); ++ivar )
    {
        ...
        //MnvH1D *mnvD  = dynamic_cast<MnvH1D*>( Hists[ivar]->Clone(h_signal) ) ;
