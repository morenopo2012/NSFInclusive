...
int main(int argc, char *argv[]){
 ...
 if(argc==1){
  ...
  const bool is_grid = false;
   ...
   //PlotUtils::MacroUtil util(reco_tree_name, mc_file_list, data_file_list, playlist, wants_truth, is_grid);
	 ...
	 universe->SetEntry(i);
          reco0++;
            ...
            //if(universe->GetInt("MasterAnaDev_targetID") ==2){
              ...
              v->m_selected_mc_reco_water.univHist(universe)->Fill(v->GetTrueValue(*universe, 0), universe->GetWeight());
              }
            }
            ...
            // CARBON
              ...
              carbon++;
              for (auto v : variables){
                v->m_selected_mc_reco_carbon.univHist(universe)->Fill(v->GetTrueValue(*universe, 0), universe->GetWeight());
              }
            }
            ...
            //else if( (cutter->IsInTrueMaterial(universe,3,26, /*anyTrakerMod*/false))){
              iron++;
              for (auto v : variables){
                v->m_selected_mc_reco_iron.univHist(universe)->Fill(v->GetTrueValue(*universe, 0), universe->GetWeight());
              }
            }
            ...
            //else if((cutter->IsInTrueMaterial(universe,3,82, /*anyTrakerMod*/false))  ){
              lead++;
              for (auto v : variables){
                v->m_selected_mc_reco_lead.univHist(universe)->Fill(v->GetTrueValue(*universe, 0), universe->GetWeight());   
              }
            }
              ...
              for (auto v : variables){
                scintillator++;
                v->m_selected_mc_reco_scintillator.univHist(universe)->Fill(v->GetTrueValue(*universe, 0), universe->GetWeight());
              }
            }
