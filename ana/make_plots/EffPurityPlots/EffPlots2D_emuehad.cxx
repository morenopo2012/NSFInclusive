...
void makeplot(string histsuffix, TFile* myfile, bool doprelimlabel=false){
  ...
  PlotUtils::MnvH2D *num = (PlotUtils::MnvH2D*)myfile->Get("h_mc_Emu_Ehad");
  PlotUtils::MnvH2D *dem = (PlotUtils::MnvH2D*)myfile->Get("h_truth_Emu_Ehad");
  
  //PlotUtils::MnvH1D *num = (PlotUtils::MnvH1D*)myfile->Get("selected_mc_reco_Emu");
  //PlotUtils::MnvH1D *dem = (PlotUtils::MnvH1D*)myfile->Get("selected_truth_reco_Emu");
  //TH1F *num = (TH1F*)f1->Get("selected_mc_reco_Emu");
  //TH1F *dem = (TH1F*)f1->Get("selected_truth_reco_Emu");
  num->Divide(num,dem);
  //num->SetBinContent(4,12,0.0);
  num->GetXaxis()->SetTitle("True Emu(GeV)");
  num->GetZaxis()->SetTitle("Efficiency");
  num->GetYaxis()->SetTitle("True Ehad (GeV)");

  applyStyle(num);
  gStyle->SetStripDecimals(0);
  //num->GetXaxis()->SetTitleOffset(1.3);
  //num->GetZaxis()->SetTitleOffset(1.5);
  num->GetXaxis()->SetRangeUser(2, 10);
  num->GetYaxis()->SetRangeUser(1.5, 10);
  ...
  if (doprelimlabel) {  
    TLatex* labl=new TLatex(1.7, 2.6,"MINER#nuA Preliminary    POT: 1.4 x10^{21}" );
...
------"<<std::endl;
  ...
  TFile *infile = new TFile(Form("%s/Hists_Efficiency_t%d_z%d_Nu_minervame1B.root",location.c_str(),targetID,targetZ));
