...
void PlotChi2Stat( MnvPlotter &mnvPlotter, MnvH1D* dataHisto, TObjArray mchistos, double dataMCScale, string var, int targetID, int targetZ, bool plotUS, bool plotDS, string playlist, string canname);
...
------"<<std::endl;
  ...
  cout<<histFileName<<endl;
    ...
    TString fileName = utils->GetHistFileName( "Energy", FileType::kAny, targetID, targetZ, helicity );
    TFile *fin = new TFile( fileName );
    assert(fin || Form("Could not open EnergyHists file") );
    
    TString BGfileName = utils->GetHistFileName( "Energy", FileType::kAny, targetID, targetZ, helicity );
    cout<<"The background file name is "<<BGfileName<<endl;
    TFile *BGfin = new TFile( BGfileName );
    assert(BGfin || Form("Could not open EnergyHists MC file") );
    TString bcksubfilename=utils->GetHistFileName("BackgroundSubtracted", FileType::kData, targetID, targetZ, helicity);
    TFile* bcksub=new TFile( bcksubfilename, "recreate");//Form("BackgroundSubtracted_t%d_z%d_%s.root",targetID,targetZ,playlist.c_str()),"recreate");
    assert(bcksub);
 ...
 for( vector<string>::iterator i = vars.begin(); i != vars.end(); ++i )
    {
       ...
       TString hname = utils->GetHistName( "sample", FileType::kAny, var.c_str(), targetID, targetZ );
        if( makeDISHists ) hname = utils->GetHistName( "sample_dis", FileType::kAny, var.c_str(), targetID, targetZ );
        ...
        cout<<"15ii9"<<endl;
        hname = utils->GetHistName( "true_sig", FileType::kAny, var.c_str(), targetID, targetZ );
        ...
        // Signal
        // Transition
        hname = utils->GetHistName( "true_trans_in_target", FileType::kAny, var.c_str(), targetID, targetZ );
        MnvH1D * mnv_true_trans_bg_target = (MnvH1D*)BGfin->Get(hname);
        hists_trans_SB.push_back( mnv_true_trans_bg_target );
        
        // Continuum
        hname = utils->GetHistName( "true_contin_in_target", FileType::kAny, var.c_str(), targetID, targetZ );
        ...
        // ===================================================
        // --- US Plastic
        hname = utils->GetHistName( "true_wrong_target_USCH", FileType::kAny, var.c_str(), targetID, targetZ );
        if( makeDISHists ) hname = utils->GetHistName( "true_dis_wrong_target_USCH", FileType::kAny, var.c_str(), targetID, targetZ );
        hists_true_wrong_target_USCH.push_back( (MnvH1D*)BGfin->Get(hname) );
        // --- DS Plastic
        hname = utils->GetHistName( "true_wrong_target_DSCH", FileType::kAny, var.c_str(), targetID, targetZ );
        if( makeDISHists ) hname = utils->GetHistName( "true_dis_wrong_target_DSCH", FileType::kAny, var.c_str(), targetID, targetZ );
        hists_true_wrong_target_DSCH.push_back( (MnvH1D*)BGfin->Get(hname) );
        // --- other target
        hname = utils->GetHistName( "true_wrong_target_Other", FileType::kAny, var.c_str(), targetID, targetZ );
        if( makeDISHists ) hname = utils->GetHistName( "true_dis_wrong_target_Other", FileType::kAny, var.c_str(), targetID, targetZ );
...
void  CalcScaleFactorAndTuneBG( MnvPlotter mnvPlotter, vector<MnvH1D*> histos_fit, vector<MnvH1D*> histos_mc_tuning, const string var, int targetID, int targetZ, double dataMCScale, TFile *scaleFactor, bool fixUS, bool fixDS, string playlist, bool makeDISPlots, const string outdir){
  ...
  // Plot before tuning plots
  PlotChi2Stat( mnvPlotter, data_sideband_untuned, hlistUntuned, dataMCScale, var.c_str(), targetID, targetZ, fixUS, fixDS, playlist, Cname);
...
*/
  ...
  //PlotChi2Stat( mnvPlotter, data_sideband, hlistTuned, dataMCScale, var.c_str(), targetID, targetZ, true, false, playlist, "US_tuned");
  PlotChi2Stat( mnvPlotter, data_sideband, hlistTuned, dataMCScale, var.c_str(), targetID, targetZ, fixUS, fixDS, playlist, Cname);
  


  TFile* tunedHists;
  if(makeDISPlots){ 
     tunedHists =  new TFile( Form("%s/TunedPlasticSidebandsDIS_%s_t%d_z%d_%s_%s.root", outdir.c_str(), !fixUS? "US":"DS", targetID, targetZ, var.c_str(), playlist.c_str(), getenv("NUKECC_TAG") ), "recreate" );
   }
  else{
     tunedHists =  new TFile( Form("%s/TunedPlasticSidebands_%s_t%d_z%d_%s_%s.root", outdir.c_str(), !fixUS? "US":"DS", targetID, targetZ, var.c_str(), playlist.c_str(), getenv("NUKECC_TAG") ), "recreate" );
   }
 ...
 */
 ...
 // if( writeChi2 && tune_fiducial  ) fprintf(Chi2Table, "%s Fiducial %s, With Sys. %.2f \n", var.c_str(), status.c_str(), chi2Sys/(Double_t)ndfStat ); 
  ...
  {
  TString cName;
    cName = Form("Ratio_%s_%s_t%d_z%02d_%s", Name.c_str(), var.c_str(), targetID, targetZ, playlist.c_str());
    cout<<"Faiza = "<<cName<<endl;
...
*/
    ...
    mnvPlotter.DrawDataMCRatio( data_cv, totalMC, dataMCScale, true, true, 0.6, 1.4, x_label.c_str(), Yaxis );
