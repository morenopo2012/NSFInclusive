...
int main(int argc, char *argv[]){
 ...
 if(argc==1){
  ...
  //GetErrorBands(util.m_mc);
   ...
   for (auto v : variablesMC) v->m_selected_trans_SB.SyncCVHistos();
   for (auto v : variablesMC) v->m_selected_contin_SB.SyncCVHistos();
   for (auto v : variablesMC) v->m_selected_mc_reco_true_target.SyncCVHistos();
  ...
  for(int t = 0; t < targetIDs.size(); t++){	  		
   ...
   for(int i=0; i<chain->GetEntries(); ++i){
	 ...
	 reco4++;
         ...
         //Select the region to fill you plastic histograms   
         Regions::idx::t_RegionIdx region = cutter->GetRegion(universe, targetIDs[t], targetZ);
	  ...
	  for (auto v : variables){
	     ...
	     v->m_selected_mc_reco.univHist(universe)->Fill(v->GetRecoValue(*universe, 0), universe->GetWeight());
           if (targetID < 10){
             if(cutter->IsInTrueMaterial(universe,targetIDs[t],targetZ,false) && cutter->PassTrueDISCut( universe )){ 
