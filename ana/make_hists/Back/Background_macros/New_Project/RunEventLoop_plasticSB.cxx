...
#include "../../include/Variable_background.h"
...
int main(int argc, char *argv[]){
  ...
  //=========================================
   ...
   if(RunCodeWithSystematics){
       histFileName = utils->GetHistFileName( "EventSelection_with_sys", FileType::kAny, targetID, targetZ, helicity ); 
     }
   else{
       histFileName = utils->GetHistFileName( "EventSelection_without_sys", FileType::kAny, targetID, targetZ, helicity );
     } 
   ...
   for (auto v : variablesMC){
      ...
      v->m_selected_mc_reco_US.SyncCVHistos();
      v->m_selected_mc_reco_DS.SyncCVHistos();
   ...
   for (auto v : variablesData) v->m_selected_data_reco_US.SyncCVHistos();
   for (auto v : variablesData) v->m_selected_data_reco_DS.SyncCVHistos();
   for (auto v : variablesData) v->m_selected_data_reco_tgt.SyncCVHistos();
   ...
   // Plotting If you want for 1D
      ...
      //PlotErrorSummary(variablesMC[i]->m_selected_mc_reco_US.hist, variablesMC[i]->GetName());
      //PlotErrorSummary(variablesMC[i]->m_selected_mc_reco_DS.hist, variablesMC[i]->GetName());
...
//   variables = {planeDNN,enu,emu};//{enu,ehad}; 
   variables = {planeDNN,enu,ehad,emu}; 
   ...
   for(int i=0; i<chain->GetEntries(); ++i){
	 ...
	 if(!PlasticBG)if(targetID<10 && universe->GetInt("NukeCC_targetID") != targetID) continue;
	 if( universe->GetVecElem("ANN_plane_probs",0) < MIN_PROB_PLANE_CUT ) continue;	   
	 reco3++; // no. of entries after ML prob cut

           if( universe->Var("planeDNN" ) < universe->GetTargetMinBin(targetID)+n ) continue;
           if( universe->Var("planeDNN" ) > universe->GetTargetMaxBin(targetID)-(n+1) ) continue;
           if( targetID == 1 && universe->Var("planeDNN") <= universe->GetTargetUSPlane(targetID) ) continue;
           reco4++; //number of entries before any cuts
...
//------------------------------------------------------------PLASTIC SB STUFF START---------------------------------------------------------------------------------------------------------------------------------------
            ...
            if( planeVal <= universe->GetTargetUSPlane(targetID) && planeVal >= universe->GetTargetDSPlane(targetID-1) && targetID < 10 ){ //-----------1   
                  us_1++; 
               if( planeVal != universe->GetTargetUSPlane(targetID) && planeVal != universe->GetTargetDSPlane(targetID-1) && planeVal != universe->GetTargetMinBin(targetID)+n && targetID < 10 ){
                      us_2++;
                  if( universe->GetTargetZStart(targetID) <= VtxZ && VtxZ <= universe->GetTargetZEnd(targetID) ){ //if its upstream bg, consider what the actual target was //--------------1a
                          us_3++;
                   if(cutter->IsInTrueMaterial(universe, targetID, targetZ)){
                     if(targetA == 56 || targetA == 55){   us_fe++;   v->m_selected_mc_US.GetComponentHist("Fe")->Fill(v->GetRecoValue(*universe, 0), wgt);
                                                               v->m_hists_US_Fe.univHist(universe)->Fill(v->GetRecoValue(*universe, 0), wgt); }
                     else if(targetA == 207){ us_pb++;    v->m_selected_mc_US.GetComponentHist("Pb")->Fill(v->GetRecoValue(*universe, 0), wgt);
                            v->m_hists_US_Pb.univHist(universe)->Fill(v->GetRecoValue(*universe, 0), wgt);}
                     else if(targetA == 12 || targetA == 14 || targetA == 16){ us_c++;   v->m_selected_mc_US.GetComponentHist("C")->Fill(v->GetRecoValue(*universe, 0), wgt);
                            v->m_hists_US_C.univHist(universe)->Fill(v->GetRecoValue(*universe, 0), wgt);}
                     else std::cout << "couldn't find target material for target " << targetID << "and Z " << targetZ <<  "; true atomic mass is = " << targetA << std::endl;
                   }
                 else{  us_other++;   v->m_selected_mc_US.GetComponentHist("Other")->Fill(v->GetRecoValue(*universe, 0), wgt);
                            v->m_hists_US_other.univHist(universe)->Fill(v->GetRecoValue(*universe, 0), wgt);}
              } //----------------1a   

                else if( universe->GetTargetZStart(targetID) > VtxZ ){ //--------------1b
                        us_4++;
                 if( targetA == 56 ){   us_other++;   v->m_selected_mc_US.GetComponentHist("Other")->Fill(v->GetRecoValue(*universe, 0), wgt);
                        v->m_hists_US_other.univHist(universe)->Fill(v->GetRecoValue(*universe, 0), wgt);}
                 else if(targetA == 207){   us_other++;   v->m_selected_mc_US.GetComponentHist("Other")->Fill(v->GetRecoValue(*universe, 0), wgt);
                        v->m_hists_US_other.univHist(universe)->Fill(v->GetRecoValue(*universe, 0), wgt);}
                 else if( cutter->IsInTrueMaterial(universe, 3, 6) || targetA == 16 || targetA == 14 ){  us_other++;  v->m_selected_mc_US.GetComponentHist("Other")->Fill(v->GetRecoValue(*universe, 0), wgt);
                        v->m_hists_US_other.univHist(universe)->Fill(v->GetRecoValue(*universe, 0), wgt);}
                else{  us_regus++; v->m_selected_mc_US.GetComponentHist("regUS")->Fill(v->GetRecoValue(*universe, 0), wgt);
                     v->m_hists_US_regUS.univHist(universe)->Fill(v->GetRecoValue(*universe, 0), wgt);
                  }
              } //-----------------1b

               else if( universe->GetTargetZEnd(targetID) < VtxZ ){ //--------------1c
                         us_5++;
                 if( targetA == 56 ){ us_other++;  v->m_selected_mc_US.GetComponentHist("Other")->Fill(v->GetRecoValue(*universe, 0), wgt); 
                       v->m_hists_US_other.univHist(universe)->Fill(v->GetRecoValue(*universe, 0), wgt);}
                 else if(targetA == 207){ us_other++;   v->m_selected_mc_US.GetComponentHist("Other")->Fill(v->GetRecoValue(*universe, 0), wgt);
                       ...
                       v->m_hists_US_other.univHist(universe)->Fill(v->GetRecoValue(*universe, 0), wgt);}
                else{  us_regds++; v->m_selected_mc_US.GetComponentHist("regDS")->Fill(v->GetRecoValue(*universe, 0), wgt);
      ...
      else if( planeVal <= universe->GetTargetUSPlane(targetID+1) && planeVal >= universe->GetTargetDSPlane(targetID) && targetID < 10 ){  //-----------------2
                     ds_1++;
             if( planeVal != universe->GetTargetUSPlane(targetID+1) && planeVal != universe->GetTargetDSPlane(targetID) && planeVal != universe->GetTargetMaxBin(targetID)-(n+1) && targetID < 10  ){
                         ds_2++;

    if( universe->GetTargetZStart(targetID) <= VtxZ && VtxZ <= universe->GetTargetZEnd(targetID) ){ //if its upstream bg, consider what the actual target was //--------------2a
                           // ds_3++;
                 if(cutter->IsInTrueMaterial(universe, targetID, targetZ)){
                     if(targetA == 56 || targetA == 55){    ds_fe++;    v->m_selected_mc_DS.GetComponentHist("Fe")->Fill(v->GetRecoValue(*universe, 0), wgt);
                           v->m_hists_DS_Fe.univHist(universe)->Fill(v->GetRecoValue(*universe, 0), wgt);}
                     else if(targetA == 207){   ds_pb++;    v->m_selected_mc_DS.GetComponentHist("Pb")->Fill(v->GetRecoValue(*universe, 0), wgt);
                           v->m_hists_DS_Pb.univHist(universe)->Fill(v->GetRecoValue(*universe, 0), wgt);}
                     else if(targetA == 12 || targetA == 14 || targetA == 16){   ds_c++;    v->m_selected_mc_DS.GetComponentHist("C")->Fill(v->GetRecoValue(*universe, 0), wgt);
                           v->m_hists_DS_C.univHist(universe)->Fill(v->GetRecoValue(*universe, 0), wgt);}
                     else std::cout << "couldn't find target material for target " << targetID << "and Z " << targetZ <<  "; true atomic mass is = " << targetA << std::endl;
                 }
                     else{   ds_other++;     v->m_selected_mc_DS.GetComponentHist("Other")->Fill(v->GetRecoValue(*universe, 0), wgt);
                           v->m_hists_DS_other.univHist(universe)->Fill(v->GetRecoValue(*universe, 0), wgt);}
              } //----------------2a   

                else if( universe->GetTargetZStart(targetID) > VtxZ ){ //--------------2b
                      ds_4++;
                 if( targetA == 56 ){    ds_other++;   v->m_selected_mc_DS.GetComponentHist("Other")->Fill(v->GetRecoValue(*universe, 0), wgt);
                          v->m_hists_DS_other.univHist(universe)->Fill(v->GetRecoValue(*universe, 0), wgt);}
                 else if(targetA == 207){    ds_other++;   v->m_selected_mc_DS.GetComponentHist("Other")->Fill(v->GetRecoValue(*universe, 0), wgt);
                          v->m_hists_DS_other.univHist(universe)->Fill(v->GetRecoValue(*universe, 0), wgt);}
                 else if( cutter->IsInTrueMaterial(universe, 3, 6) || targetA == 16 || targetA == 14 ){    ds_other++;   v->m_selected_mc_DS.GetComponentHist("Other")->Fill(v->GetRecoValue(*universe, 0), wgt);
                          v->m_hists_DS_other.univHist(universe)->Fill(v->GetRecoValue(*universe, 0), wgt);}
                else{   ds_regus++;    v->m_selected_mc_DS.GetComponentHist("regUS")->Fill(v->GetRecoValue(*universe, 0), wgt);
                          v->m_hists_DS_regUS.univHist(universe)->Fill(v->GetRecoValue(*universe, 0), wgt);}
              } //-----------------2b

               else if( universe->GetTargetZEnd(targetID) < VtxZ ){ //--------------2c
                       ds_5++;
                 if( targetA == 56 ){    ds_other++;   v->m_selected_mc_DS.GetComponentHist("Other")->Fill(v->GetRecoValue(*universe, 0), wgt);
                         v->m_hists_DS_other.univHist(universe)->Fill(v->GetRecoValue(*universe, 0), wgt);}
                 else if(targetA == 207){    ds_other++;   v->m_selected_mc_DS.GetComponentHist("Other")->Fill(v->GetRecoValue(*universe, 0), wgt);
                         ...
                         v->m_hists_DS_other.univHist(universe)->Fill(v->GetRecoValue(*universe, 0), wgt);}
                else{ ds_regds++;     v->m_selected_mc_DS.GetComponentHist("regDS")->Fill(v->GetRecoValue(*universe, 0), wgt);
      ...
      else if(planeVal == universe->GetTargetPlane(targetID) && targetID < 10){ //--------------3                
                tgt_1++;
    if( universe->GetTargetZStart(targetID) <= VtxZ && VtxZ <= universe->GetTargetZEnd(targetID) ){ //if its upstream bg, consider what the actual target was  //--------------3a
                     tgt_2++;
                 if(cutter->IsInTrueMaterial(universe, targetID, targetZ)){
                     if(targetA == 56 || targetA == 55){ v->m_selected_mc_tgt.GetComponentHist("Fe")->Fill(v->GetRecoValue(*universe, 0), wgt);
                          v->m_hists_tgt_Fe.univHist(universe)->Fill(v->GetRecoValue(*universe, 0), wgt);}
                     else if(targetA == 207){ v->m_selected_mc_tgt.GetComponentHist("Pb")->Fill(v->GetRecoValue(*universe, 0), wgt);
                          v->m_hists_tgt_Pb.univHist(universe)->Fill(v->GetRecoValue(*universe, 0), wgt);}
                     else if(targetA == 12 || targetA == 14 || targetA == 16){ v->m_selected_mc_tgt.GetComponentHist("C")->Fill(v->GetRecoValue(*universe, 0), wgt);
                          v->m_hists_tgt_C.univHist(universe)->Fill(v->GetRecoValue(*universe, 0), wgt);}
                     else std::cout << "couldn't find target material for target " << targetID << "and Z " << targetZ <<  "; true atomic mass is = " << targetA << std::endl;
                 }
                     else{ v->m_selected_mc_tgt.GetComponentHist("Other")->Fill(v->GetRecoValue(*universe, 0), wgt);
                          v->m_hists_tgt_other.univHist(universe)->Fill(v->GetRecoValue(*universe, 0), wgt);}
              } //----------------3a   

                else if( universe->GetTargetZStart(targetID) > VtxZ ){  //--------------3b
                    tgt_3++;
                 if( targetA == 56 ){ v->m_selected_mc_tgt.GetComponentHist("Other")->Fill(v->GetRecoValue(*universe, 0), wgt);
                          v->m_hists_tgt_other.univHist(universe)->Fill(v->GetRecoValue(*universe, 0), wgt);}
                 else if(targetA == 207){ v->m_selected_mc_tgt.GetComponentHist("Other")->Fill(v->GetRecoValue(*universe, 0), wgt);
                          v->m_hists_tgt_other.univHist(universe)->Fill(v->GetRecoValue(*universe, 0), wgt);}
                 else if( cutter->IsInTrueMaterial(universe, 3, 6) || targetA == 16 || targetA == 14 ){ v->m_selected_mc_tgt.GetComponentHist("Other")->Fill(v->GetRecoValue(*universe, 0), wgt);
                          v->m_hists_tgt_other.univHist(universe)->Fill(v->GetRecoValue(*universe, 0), wgt);}
                else{ v->m_selected_mc_tgt.GetComponentHist("regUS")->Fill(v->GetRecoValue(*universe, 0), wgt);
                          v->m_hists_tgt_regUS.univHist(universe)->Fill(v->GetRecoValue(*universe, 0), wgt);}
              } //-----------------3b

               else if( universe->GetTargetZEnd(targetID) < VtxZ ){  //--------------3c
             tgt_4++;
                 if( targetA == 56 ){ v->m_selected_mc_tgt.GetComponentHist("Other")->Fill(v->GetRecoValue(*universe, 0), wgt);
                       v->m_hists_tgt_other.univHist(universe)->Fill(v->GetRecoValue(*universe, 0), wgt);}
                 else if(targetA == 207){ v->m_selected_mc_tgt.GetComponentHist("Other")->Fill(v->GetRecoValue(*universe, 0), wgt);
                        ...
                        v->m_hists_tgt_other.univHist(universe)->Fill(v->GetRecoValue(*universe, 0), wgt);}
                else{ v->m_selected_mc_tgt.GetComponentHist("regDS")->Fill(v->GetRecoValue(*universe, 0), wgt);
...
//------------------------------------------------------------PLASTIC SB STUFF END-------------------
	 ...
	 if(!PlasticBG)if(targetID<10 && dataverse->GetInt("NukeCC_targetID") != targetID) continue;
	 if( dataverse->GetVecElem("ANN_plane_probs",0) < MIN_PROB_PLANE_CUT ) continue;	   
         reco3++;
	
           if( dataverse->Var("planeDNN" ) < dataverse->GetTargetMinBin(targetID)+n ) continue;
           if( dataverse->Var("planeDNN" ) > dataverse->GetTargetMaxBin(targetID)-(n+1) ) continue;
         reco0++;
...
//--------------------------------------------------------Plactic SB Data Histograms--------------------
      ...
      if( planeVal <= dataverse->GetTargetUSPlane(targetID) && planeVal >= dataverse->GetTargetDSPlane(targetID-1) && targetID < 10 ){ //-----------1
             if( planeVal != dataverse->GetTargetUSPlane(targetID) && planeVal != dataverse->GetTargetDSPlane(targetID-1) && planeVal != dataverse->GetTargetMinBin(targetID)+n && targetID < 10 ){
                   data_US++;   v->m_selected_data_reco_US.hist->Fill(v->GetRecoValue(*dataverse, 0));//  h[kdata_US]->hist->Fill( var ); 
              }
       }   //------------------1

      else if( planeVal <= dataverse->GetTargetUSPlane(targetID+1) && planeVal >= dataverse->GetTargetDSPlane(targetID) && targetID < 10 ){  //-----------------2
             if( planeVal != dataverse->GetTargetUSPlane(targetID+1) && planeVal != dataverse->GetTargetDSPlane(targetID) && planeVal != dataverse->GetTargetMaxBin(targetID)-(n+1) && targetID < 10  ){
               data_DS++;   v->m_selected_data_reco_DS.hist->Fill(v->GetRecoValue(*dataverse, 0));//   h[kdata_DS]->hist->Fill( var );
              }             
       }//---------------2
      else if(planeVal == dataverse->GetTargetPlane(targetID) && targetID < 10){ //--------------3
             data_tgt++;   v->m_selected_data_reco_tgt.hist->Fill(v->GetRecoValue(*dataverse, 0));// h[kdata_tgt]->hist->Fill( var );
       }//---------------3
      else{
...
for(auto band : error_bands){
   ...
   std::cout<<" SB only = "<<reco0<<std::endl;
   ...
   /*std::cout<<" US third cut = "<<us_3<<std::endl;
   std::cout<<" US fourth cut = "<<us_4<<std::endl;
   std::cout<<" US fifth cut = "<<us_5<<std::endl;
   std::cout<<" US sixth cut = "<<us_6<<std::endl;*/
...
/*   std::cout<<" DS third cut = "<<ds_3<<std::endl;
   std::cout<<" DS fourth cut = "<<ds_4<<std::endl;
   std::cout<<" DS fifth cut = "<<ds_5<<std::endl;
   std::cout<<" DS sixth cut = "<<ds_6<<std::endl;
   std::cout<<" TGT first cut = "<<tgt_1<<std::endl;
   std::cout<<" TGT second cut = "<<tgt_2<<std::endl;
   std::cout<<" TGT third cut = "<<tgt_3<<std::endl;
   std::cout<<" TGT fourth cut = "<<tgt_4<<std::endl;
   std::cout<<" TGT fifth cut = "<<tgt_5<<std::endl;
   std::cout<<" last cut = "<<end_1<<std::endl;
 */
   std::cout<<"**********************************"<<std::endl;
