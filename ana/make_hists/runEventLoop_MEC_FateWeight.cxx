...
void FillVariable( PlotUtils::ChainWrapper* chain, HelicityType::t_HelicityType helicity, NukeCCUtilsNSF *utils , NukeCC_Cuts *cutter ,NukeCC_Binning  *binsDef ,std::vector<Var*>& variables,std::vector<Var2D*>& variables2d,bool isMC, int targetID=1, int targetZ=26, const string playlist="minervame1A", bool doDIS=true, bool isDSCAL= false);
...
int main(int argc, char *argv[]){
    ...
    //Oscar setup to run for different configurations
    bool isDSCAL = false; //Set true for DSCAL cuts, set false for Tracker

    TFile fout(dir.Append(histFileName),"RECREATE");
    std::cout<<"helicity: "<<helicity<<std::endl;
    // For 1D variables
    FillVariable(chainMC, helicity, utils, cutter,binsDef,variablesMC,variables2DMC,true,targetID, targetZ, playlist,doDIS, isDSCAL);
    ...
    FillVariable(chainData, helicity, utils, cutter,binsDef,variablesData,variables2DData,false,targetID, targetZ, playlist,doDIS, isDSCAL);
...
void FillVariable( PlotUtils::ChainWrapper* chain, HelicityType::t_HelicityType helicity, NukeCCUtilsNSF *utils , NukeCC_Cuts *cutter ,NukeCC_Binning  *binsDef ,std::vector<Var*>& variables,std::vector<Var2D*>& variables2d,bool isMC,int targetID, int targetZ, const string playlist, bool doDIS, bool isDSCAL){
  ...
  Var* Eavail;
  if(isDSCAL) { Eavail = new Var("Eavailable", "Eavail (GeV)", Eavbin, &CVUniverse::GetECALHCALAvEnergy, &CVUniverse::GetEavailTrue);}
  if(!isDSCAL) { Eavail = new Var("Eavailable", "Eavail (GeV)", Eavbin, &CVUniverse::GetTrackerECALAvEnergy, &CVUniverse::GetEavailTrue); } //GetTrackerECALAvEnergy
    ...
    if(isMC){
      ...
      std::vector<CVUniverse*> error_band_universes = band.second;
        for (auto universe : error_band_universes){
	      ...
	      universe->SetEntry(i);
        reco0++;
        if(!cutter->PassReco(universe,helicity)) continue; //Helicity, Hexagon, dead time, Enu cut, curve and coil muon cuts.
        reco1++;
        ...
        double vtxz = universe->GetVecElem("vtx",2);
        double minZ, maxZ;
        if(isDSCAL){
          minZ = 8590.0; //85 plane 2
          maxZ = 8916.0; //91 plane 2
        }
        else{
          minZ = 6990.0; 
          maxZ = 8340.0; 
        }
        if(vtxz < minZ || vtxz > maxZ) continue; //Cut on the vertex Z position
        //MEC cuts
        bool cuts = cutter->passesAll(universe); //IsFiducial (Useless), HadronicEnergy between 2 and 20 GeV, Muon momentum > 1.5 GeV
        if(!cuts) continue;
	      reco4++;
...
/*
    ...
    if(!isMC){
      ...
      double vtxz;
      vtxz = dataverse->GetVecElem((dataverse->GetAnaToolName() + "_vtx").c_str(),2);
      double minZ, maxZ;
       if(isDSCAL){
          minZ = 8590.0; //85 plane 2
          maxZ = 8916.0; //91 plane 2
      ...
      if(vtxz < minZ || vtxz > maxZ) continue;
      //MEC cuts
