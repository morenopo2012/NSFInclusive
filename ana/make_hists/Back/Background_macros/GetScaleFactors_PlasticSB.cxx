...
------"<<std::endl;
  ...
  const string playlist= argv[4];
   ...
   if(RunCodeWithSystematics){
       ...
       histFileName = Form("%s/Hists_PlasticSideband_t%d_z%02d_Nu.root", outdir.c_str(), targetID, targetZ);
     }
   else{
       histFileName = Form("%s/Hists_PlasticBackgd_without_t%d_z%02d_Nu.root", outdir.c_str(), targetID, targetZ);
     } 
 ...
 if(var == "planeDNN"){
  ...
  if( ! tune_fiducial ){
    scaleFactor = new TFile( Form("%s/CHScaleFactors_t%d_z%d_%s.root", outdir.c_str(), targetID, targetZ, playlist.c_str() ), "recreate" );
    assert(scaleFactor);
  }
