...
void makeplot(string histsuffix, TFile* myfile, bool doprelimlabel=false){
  ...
  PlotUtils::MnvH2D *num = (PlotUtils::MnvH2D*)myfile->Get("selected_mc_response2d_Emu_Ehad_migration");
  //PlotUtils::MnvH2D *num = (PlotUtils::MnvH2D*)myfile->Get("selected_mc_response1d_Emu_migration");
  //PlotUtils::MnvH2D *dem = (PlotUtils::MnvH2D*)myfile->Get("selected_truth2d_reco_Emu_Ehad");
  
  //TH1F *num = (TH1F*)f1->Get("selected_mc_reco_Emu");
  //TH1F *dem = (TH1F*)f1->Get("selected_truth_reco_Emu");
  //num->Divide(num,dem);
  num->SetBinContent(4,12,0.0);
  num->GetXaxis()->SetTitle("Reco Emu vs Ehad(GeV)");
  num->GetYaxis()->SetTitle("True Emu vs Ehad (GeV)");
  //num->GetXaxis()->SetTitle("Reco Emu(GeV)");
  //num->GetYaxis()->SetTitle("True Emu (GeV)");
...
------"<<std::endl;
  ...
  TFile *infile = new TFile(Form("%s/Hists_Migration_t%d_z%d_Nu_minervame1D.root",location.c_str(),targetID,targetZ));
