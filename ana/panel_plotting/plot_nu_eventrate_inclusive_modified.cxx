...
void makePlots(bool doMultipliers,bool doRatio, bool doBkgOnly, bool zoom, string location, string varcombo, string variable)
{
  ...
  MnvH2D* mcMnv=(MnvH2D*)f3.Get(Form("h_%s_mc",varcombo.c_str()));//Get from N track
  MnvH2D* dataMnv = (MnvH2D*)f3.Get(Form("h_%s_data",varcombo.c_str()));//Get from N track

  MnvH2D* mcMnv_qe = (MnvH2D*)f3.Get(Form("h_%s_qe",varcombo.c_str()));//Get from N track
  MnvH2D* mcMnv_res = (MnvH2D*)f3.Get(Form("h_%s_res",varcombo.c_str()));//Get from N track
  MnvH2D* mcMnv_dis = (MnvH2D*)f3.Get(Form("h_%s_dis",varcombo.c_str()));//Get from N track
  MnvH2D* mcMnv_dis_dis = (MnvH2D*)f3.Get(Form("h_%s_true_dis",varcombo.c_str()));//Get from N track
  MnvH2D* mcMnv_dis_sis = (MnvH2D*)f3.Get(Form("h_%s_sis",varcombo.c_str()));//Get from N track
  MnvH2D* mcMnv_2p2h = (MnvH2D*)f3.Get(Form("h_%s_2p2h",varcombo.c_str()));//Get from N track
  MnvH2D* mcMnv_oth = (MnvH2D*)f3.Get(Form("h_%s_oth",varcombo.c_str()));//Get from N track
  MnvH2D* mcMnv_NC = (MnvH2D*)f3.Get(Form("h_%s_NC",varcombo.c_str()));//Get from N track
  MnvH2D* mcMnv_NC_Bkg = (MnvH2D*)f3.Get(Form("h_%s_NC_Bkg",varcombo.c_str()));//Get from N track
  MnvH2D* mcMnv_NC_WrongSign = (MnvH2D*)f3.Get(Form("h_%s_Bkg_Wrong_Sign",varcombo.c_str()));//Get from N track
  dataMnv->GetXaxis()->SetTitle("W ");
  cout << "Doing stuff now" << endl;
  ...
  TH2* mc_totalerr=new TH2D(mcMnv->GetCVHistoWithError());
  ...
  mc->SetLineWidth(2);
  mc_totalerr->SetLineColor(kRed);
  mc_totalerr->SetFillColor(kBlue);
  mc_totalerr->SetFillStyle(3224);
  mc_totalerr->SetMarkerStyle(1);
  mc_totalerr->SetMarkerColor(kRed);
  ...
  if(!doBkgOnly){
    ...
    if(!doRatio && variable!="W")histAndOpts.push_back(std::make_pair(mc_totalerr,       "graph0e3"));
  ...
  GridCanvas* gc=plotXAxis1D(histAndOpts, Form("%s",variable.c_str()), "Q^{2}", 5,4,800,500,doMultipliers ? &multipliers[0] : NULL);
  ...
  if(doRatio){
    ...
    gc->SetYTitle("Events (x10^{-5}) per units^{2}");
  }
  gc->Modified();
  ...
  // Values to multiply each bin by to get them on a similar range
  vector<double> multipliers2 = GetScales(histAndOpts, false, 14.99,0.65);
  // multipliers2[1] = 0.15;

  // plotpz1D fiddles the x axis values to squash up the tail so it
  // doesn't take up all the horizontal space.
  //GridCanvas* gc2=plotYAxis1D(histAndOpts, "Q^{2}", Form("%s",variable.c_str()),4,4,800,500,doMultipliers ? &multipliers2[0] : NULL);
  GridCanvas* gc2=plotYAxis1D(histAndOpts, "Q^{2}", Form("%s",variable.c_str()),6,3,800,500,doMultipliers ? &multipliers2[0] : NULL);
  gc2->SetLogx(true);
  if(doRatio){
    ...
    if(doMultipliers)gc2->SetYLimits(0, 14.99);
    else gc2->SetYLimits(0, 14.99);
    gc2->SetYTitle("Events (x10^{-5}) per units^{2}");    
  }
  gc2->Modified();
  //  prelim->Draw("SAME");
  if(doRatio){
...
int main(int argc, char* argv[])
{
  ...
  string varcar = argv[2];
  string variablename = argv[3];

    makePlots(false,true,false,false,location,varcar,variablename);
  makePlots(true,false,false,false,location,varcar,variablename);
  makePlots(true,false,true,false,location,varcar,variablename);
  makePlots(false,true,true,false,location,varcar,variablename);
  makePlots(false,true,true,true,location,varcar,variablename);
  makePlots(false,false,false,false,location,varcar,variablename);
