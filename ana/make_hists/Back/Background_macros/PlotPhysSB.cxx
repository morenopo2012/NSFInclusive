...
int main(int argc, char * argv[]){
   ...
   if(RunCodeWithSystematics){
       histFileName = Form("%s/Hists_PhysicsBackgd_with_SYS_FullDet_q2WfromBranch_ME1A_targetsCombined_t%d_z%02d_Nu.root", outdir.c_str(), targetID, targetZ ); 
     }
   else{
...
for( vector<string>::iterator i = vars.begin(); i != vars.end(); ++i ){
   const string& var = *i;
    ...
    TString histFilename = Form("%s/TunedPhysicsSidebands_t%d_z%02d_%s_%s.root", outdir.c_str(), targetID, targetZ, var.c_str(), playlist.c_str());
    TFile *f_tuned = new TFile( histFilename,"read" );
   f1->cd();
   hists_Trans_data.push_back(      (MnvH1D*)f_tuned->Get(Form("selected_data_reco_trans_%s", var.c_str())));
   ...
   hists_Contin_data.push_back(      (MnvH1D*)f_tuned->Get(Form("selected_data_reco_contin_%s", var.c_str())));
   ...
   MnvH1D *hists_Trans_data        = (MnvH1D*)f_tuned->Get(Form("selected_data_reco_trans_%s", var.c_str()));
   ...
   MnvH1D *hists_Signal_in_Trans = (MnvH1D*)f1->Get(Form("hists_signal_in_trans_%s",  var.c_str()));
    ...
    cout<<"Variable = "<<var.c_str()<<"Data = "<< hists_Trans_data->GetBinContent(2)<<endl;  
   ...
   MnvH1D *hists_Contin_data      = (MnvH1D*)f_tuned->Get(Form("selected_data_reco_contin_%s", var.c_str()));
  ...
  PlotChi2Stat( mnvPlotter, hists_Contin_data, hlistUntunedContin, dataMCScale, var.c_str(), targetID, targetZ, false, true, playlist, "Contin_Untuned");
  
  hists_Trans_data->Reset();
  hists_Contin_data->Reset();
   }
...
for( unsigned int i = 0; i != vars.size(); ++i ){
  ...
  histos_mc_trans.push_back( (MnvH1D*)f_tuned->Get(Form("selected_data_reco_trans_%s", var.c_str())));
  ...
  histos_mc_trans.push_back( hists_Contin_in_Trans[i] );
    cout<<"Variable = "<<var.c_str()<<"Data = "<< hists_Trans_data[i]->GetBinContent(2)<<endl;  
  //MnvH1D* data_sideband_Contin = hists_Contin_data[i];
...
// Now I am going to make tuned plots!!!! 
  ...
  for( vector<string>::iterator i = vars.begin(); i != vars.end(); ++i ){  
    ...
    hists_Signal_in_Trans_tuned.push_back(  (MnvH1D*)f_tuned->Get(Form("trans_tuned_grouped_t%02d_z%02d_%s_%s_Signal", targetID, targetZ, trueZ.c_str(), var.c_str() )));
    hists_Trans_in_Trans_tuned.push_back(   (MnvH1D*)f_tuned->Get(Form("trans_tuned_grouped_t%02d_z%02d_%s_%s_Trans",  targetID, targetZ, trueZ.c_str(), var.c_str() )));
    hists_Contin_in_Trans_tuned.push_back(  (MnvH1D*)f_tuned->Get(Form("trans_tuned_grouped_t%02d_z%02d_%s_%s_Contin", targetID, targetZ, trueZ.c_str(), var.c_str() )));
 
    hists_Contin_data_tuned.push_back(       (MnvH1D*)f_tuned->Get(Form("selected_data_reco_contin_%s", var.c_str())));
    hists_Signal_in_Contin_tuned.push_back(  (MnvH1D*)f_tuned->Get(Form("contin_tuned_grouped_t%02d_z%02d_%s_%s_Signal", targetID, targetZ, trueZ.c_str(), var.c_str() )));
    hists_Trans_in_Contin_tuned.push_back(   (MnvH1D*)f_tuned->Get(Form("contin_tuned_grouped_t%02d_z%02d_%s_%s_Trans",  targetID, targetZ, trueZ.c_str(), var.c_str() )));
    hists_Contin_in_Contin_tuned.push_back(  (MnvH1D*)f_tuned->Get(Form("contin_tuned_grouped_t%02d_z%02d_%s_%s_Contin", targetID, targetZ, trueZ.c_str(), var.c_str() )));
{
    f_tuned->cd();
    MnvH1D *hists_Trans_data_tuned      = (MnvH1D*)f_tuned->Get(Form("selected_data_reco_trans_%s", var.c_str()));
    MnvH1D *hists_Signal_in_Trans_tuned = (MnvH1D*)f_tuned->Get(Form("trans_tuned_grouped_t%02d_z%02d_%s_%s_Signal", targetID, targetZ, trueZ.c_str(), var.c_str() ));
    MnvH1D *hists_Trans_in_Trans_tuned  = (MnvH1D*)f_tuned->Get(Form("trans_tuned_grouped_t%02d_z%02d_%s_%s_Trans",  targetID, targetZ, trueZ.c_str(), var.c_str() ));
    MnvH1D *hists_Contin_in_Trans_tuned = (MnvH1D*)f_tuned->Get(Form("trans_tuned_grouped_t%02d_z%02d_%s_%s_Contin", targetID, targetZ, trueZ.c_str(), var.c_str() ));
    ...
    MnvH1D *hists_Signal_in_Contin_tuned = (MnvH1D*)f_tuned->Get(Form("contin_tuned_grouped_t%02d_z%02d_%s_%s_Signal", targetID, targetZ, trueZ.c_str(), var.c_str() ));
    MnvH1D *hists_Trans_in_Contin_tuned  = (MnvH1D*)f_tuned->Get(Form("contin_tuned_grouped_t%02d_z%02d_%s_%s_Trans",  targetID, targetZ, trueZ.c_str(), var.c_str() ));
    MnvH1D *hists_Contin_in_Contin_tuned = (MnvH1D*)f_tuned->Get(Form("contin_tuned_grouped_t%02d_z%02d_%s_%s_Contin", targetID, targetZ, trueZ.c_str(), var.c_str() ));
    ...
    PlotChi2Stat( mnvPlotter, hists_Contin_data_tuned, hlisttunedContin, dataMCScale, var.c_str(), targetID, targetZ, false, true, playlist, "Contin_tuned");

  //hists_Trans_data_tuned->Reset();
  //hists_Contin_data_tuned->Reset();
  }
}
  ...
  for( unsigned int i = 0; i != vars.size(); ++i ){  
    ...
    PlotBGStuff(mnvPlotter, histos_mc_trans_tuned, vars[i], targetID, targetZ, dataMCScale, true, false, playlist, "Tuned" );   //Filling US sidebands
    PlotBGStuff(mnvPlotter, histos_mc_contin_tuned, vars[i], targetID, targetZ, dataMCScale, false, true, playlist, "Tuned" );   //Filling DS sidebands
...
void PlotBGStuff(MnvPlotter mnvPlotter, vector<MnvH1D*> histos_mc, const string var, int targetID, int targetZ, double dataMCScale, bool plotUS, bool plotDS, string playlist, string Name){
  ...
  //TString histFilename = Form("%s/TunedPhysicsSidebands_t%d_z%02d_%s_%s.root", outdir.c_str(), targetID, targetZ, var.c_str(), playlist.c_str());
  //TFile *f_tuned = new TFile( histFilename,"read" );
  //MnvH1D* data_sideband = ((MnvH1D*)f_tuned->Get(Form("selected_data_reco_trans_%s", var.c_str())));
