...
void makePlots(bool doMultipliers,bool doGenies,string location)
{
  ...
  TFile f1(Form("%s_CV/CrossSection_per_nucleon_iterations_4_SpecialSampleIncluded.root_pzmu.root",location.c_str()));//Final result
  TFile f2(Form("%s_CV/CrossSection_per_nucleon_iterations_4_SpecialSampleIncluded.root_pzmu.root",location.c_str()));//Default GENIE
  TFile f3(Form("%s_CV/CrossSection_per_nucleon_iterations_4_SpecialSampleIncluded.root_pzmu.root",location.c_str()));//GENIE+2p2h+RPA (aka no tune)
  ...
  double multipliers[]={30, 8, 3, 2,1,
			1, 1, 1, 2, 4,
			20,100,1000};
  GridCanvas* gc=plotXAxis1D(histAndOpts, "P_{||}", "Pt",5,3,800,500, doMultipliers ? multipliers : NULL);
  ...
  GridCanvas* gc2=plotYAxis1D(histAndOpts, "Pt", "P_{||}",4,3,800,500,doMultipliers ? multipliers2 : NULL);
  gc2->SetYLimits(0, 4.59);
