...
//        mcxBjTable.open(Form("Event_table_z%02d.tex", -1), std::ofstream::out); 
  ...
  string targetString;  
  if( targetZ == 6 )
    targetString = "Carbon of Target 3";
  else if( targetZ == 26 )
    targetString = "Iron of target 1";
  else if( targetZ == 82 )
    targetString = "Lead";
  if( targetID == 14 )
    targetString = "Tracker";
...
//////////////For before background plots /////////////////////////////
	  ...
	  sig2->SaveAs(Form("signal_%s.png", var.c_str() ));
            TString cName = Form("Inclusive_BeforeBG_dataMC_%s_%s_%s", var.c_str(), suffix.c_str(), playlist.c_str() );
	    ...
	    //mnvPlotter.DrawDataMCWithErrorBand( mnvDBS, mnvMCBS, dataMCScale, legPos, false, mnvNonDIS, mnvBGD, false, true);
            ...
            if(ADD_HISTO_TITLES) mnvPlotter.AddHistoTitle( Form( "DIS - %s", targetString.c_str() ), titleSize );
            mnvPlotter.MultiPrint( &cDIS );
        }

     /*   {
            
            TString cName = Form( "Inclusive_BeforeBG_dataMCRatio_%s_%s_%s", var.c_str(), suffix.c_str(), playlist.c_str() );
	    ...
	    mnvPlotter.DrawDataMCRatio( mnvD, mnvMC, dataMCScale, true, true, 0.6, 1.7, x_label.c_str(), Yaxis );
