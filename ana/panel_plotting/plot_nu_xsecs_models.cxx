...
void makePlots(bool doMultipliers,string location)
{
  ...
  TFile f(Form("%s_CV/CrossSection_per_nucleon_iterations_4_SpecialSampleIncluded.root_pzmu.root",location.c_str()));//CV MC
  TFile f2("/minerva/app/users/drut1186/cmtuser/Minerva_v10r8p9_CCQENu2DLE_hadronv2/Ana/CCQENu2DLE/ana/plot_macros_pub/CrossSections/GENIEModels.root");//GENIE Models you get from the $CCQENU2DROOT/ana/plots_macro_pub/ExtractModelCrossSections
  ...
  histAndOpts.push_back(std::make_pair(data,     "graph ep"));//9
  ...
  gc->Modified();
  gc->Print(doMultipliers ? "nu-2d-xsec-models-pt-multiplier.eps" : "nu-2d-xsec-models-pt.eps");
  gc->Print(doMultipliers ? "nu-2d-xsec-models-pt-multiplier.png" : "nu-2d-xsec-models-pt.png");
  gc->Print(doMultipliers ? "nu-2d-xsec-models-pt-multiplier.C" : "nu-2d-xsec-models-pt.C");
  ...
  for(int i=0;i<histAndOpts.size()-4;i++){
    ...
    // Set the y range manually. Can also use gc->Remax() to guess automatically
    //  gc->SetYLimits(0, 3.9);
    gc_sub->SetYLimits(0, 1.99);
    gc_sub->SetYTitle("Ratio to Default GENIE");
    gc_sub->Modified();
    gc_sub->Print(doMultipliers ? Form("nu-2d-xsec-models-pt-multiplier-group-%d.eps",i) : Form("nu-2d-xsec-models-pt-group-%d.eps",i));
    gc_sub->Print(doMultipliers ? Form("nu-2d-xsec-models-pt-multiplier-group-%d.PNG",i) : Form("nu-2d-xsec-models-pt-group-%d.png",i));
    gc_sub->Print(doMultipliers ? Form("nu-2d-xsec-models-pt-multiplier-group-%d.C",i) : Form("nu-2d-xsec-models-pt-group-%d.C",i));
  ...
  for(int i=0;i<histAndOpts.size()-4;i++){
    ...
    // Set the y range manually. Can also use gc->Remax() to guess automatically
    //  gc->SetYLimits(0, 3.9);
    gc2_sub->SetYLimits(0, 1.99);
    gc2_sub->SetYTitle("Ratio to Default GENIE");
    gc2_sub->Modified();

    gc2_sub->Print(doMultipliers ? Form("nu-2d-xsec-models-pz-multiplier-group-%d.eps",i) : Form("nu-2d-xsec-models-pz-group-%d.eps",i));
    gc2_sub->Print(doMultipliers ? Form("nu-2d-xsec-models-pz-multiplier-group-%d.PNG",i) : Form("nu-2d-xsec-models-pz-group-%d.png",i));
    gc2_sub->Print(doMultipliers ? Form("nu-2d-xsec-models-pz-multiplier-group-%d.C",i) : Form("nu-2d-xsec-models-pz-group-%d.C",i));
