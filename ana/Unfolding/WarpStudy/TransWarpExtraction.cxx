...
namespace PlotUtils
{
  template <class MnvH> TransWarpExtraction<MnvH>::TransWarpExtraction( string data_file, string data_name, 
                                                                        ...
                                                                        int stat_universes, bool bIterLogScale )
  {
  ...
  template <class MnvH> void TransWarpExtraction<MnvH>::MakeTruncatedChi2Dists( double nStdDev )
  {
    ...
    for(uint iter = 1; iter <=  m_iterations.size(); ++iter)
    {
      std::vector<double> chi2_iter_values;
      for(uint su = 1; su <= m_nStatUniverses; ++su) chi2_iter_values.push_back( m_chi2_md_td_iter_stat->GetBinContent(iter,su) );
  ...
  template <class MnvH> void TransWarpExtraction<MnvH>::FillChi2Dists( int stat_uni, MnvH* h_data_unfolded, MnvH* input_truth, MnvH* unfoldtruth, int num_iter )
  {
    ...
    if(m_chi2_md_md_iter_chi2)
    {
      ...
      //if(chi2_md_td<0) printf("CHI2: %5.3e\n",chi2_md_td);
...
int main( int argc, char **argv)
{
  ...
  const char* const short_options = "o:D:d:I:i:M:m:R:r:T:t:n:lLu:z:c:C:p:P:x:vVh";
  ...
  switch(num_dim){
    case 1: 
    {
      TransWarpExtraction< MnvH1D > excelsior(data_file, data, data_truth_file, data_truth, reco_file, reco, truth_file, truth, migration_file, migration, iterations, exclude_chi2_bins, num_uni, bIterLogScale );
    ...
    case 2: 
    {
      TransWarpExtraction< MnvH2D > excelsior(data_file, data, data_truth_file, data_truth, reco_file, reco, truth_file, truth, migration_file, migration, iterations, exclude_chi2_bins, num_uni, bIterLogScale );
      excelsior.ClearInputErrorBands();
      excelsior.ScalePOT( pot_scale ); 
      excelsior.ScaleDataPOT( data_pot_norm ); 
      excelsior.SetChi2MaxAndStep( max_chi2, step_chi2 );
    ...
    case 3: 
    {
      TransWarpExtraction< MnvH3D > excelsior(data_file, data, data_truth_file, data_truth, reco_file, reco, truth_file, truth, migration_file, migration, iterations, exclude_chi2_bins, num_uni, bIterLogScale );
      excelsior.ClearInputErrorBands();
      excelsior.ScalePOT( pot_scale ); 
      excelsior.ScaleDataPOT( data_pot_norm ); 
      excelsior.SetChi2MaxAndStep( max_chi2, step_chi2 );
