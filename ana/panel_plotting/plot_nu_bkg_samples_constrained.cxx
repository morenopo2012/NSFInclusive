...
void makePlots(bool doMultipliers, string location,string sample, bool doRatio, bool scaledQELike)
{
  ...
  */
  ...
  recoil3Dbins.push_back(799.99);

  for(int i=0;i<recoil3Dbins.size();i++) recoil3Dbins[i]/=1000.0;
  */
  ...
  leg->AddEntry(mcresults, "Minerva Tune v1", "lf");
  leg->AddEntry(mcresultsscaled, "Tune v1 w/ sc. bkg.", "lf");
  ...
  GridCanvas* gc1=NULL;
  gc1=plotXAxis1D_ReducedXRange(histAndOpts, "Visible Energy (MeV)","P_{t}", 0,800,doMultipliers ? &multi_1[0] : NULL);
  ...
  if(doRatio)gc1->SetYTitle("Ratio to scaled Minerva Tune v1");
  else gc1->SetYTitle("Events per (MeV#timesGeV^{2})/c^{2}");
  mytex.DrawLatex(0.22,0.95,mystring.c_str());
  leg->Draw("SAME");
  gc1->Modified();
