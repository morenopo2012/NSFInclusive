...
#playlists=["minervame6A","minervame6C"]
    ...
    for j in range(0,len(tunes)):
        for k in range(0,len(stages)):
            ...
            if i==0 and \
               ...
               k==0:
                ...
                cmd = "python SubmitPlaylistToGrid_antiNu.py --outdir "+outdir+" --playlist "+playlists[i]+" "+tunes[j]+" --tag "+tag_name+" --stage"+" "+stages[k]+" "+"--sample Signal --notimestamp 
		os.system(cmd)
		...
		for things in _list:
			...
			break
                ...
                tune_tag = tune__tag.replace(" ","")
                tag_name = tune_tag
                cmd = "python SubmitPlaylistToGrid_antiNu.py --outdir "+ outdir+" --playlist "+playlists[i]+" "+tunes[j]+" --tag "+tag_name+" --stage"+" "+stages[k]+" "+"--sample Signal --sametar --tarfilename "+tarfilename
            	os.system(cmd)
        #print tag_name
        #print cmd
	#print tarfilename
