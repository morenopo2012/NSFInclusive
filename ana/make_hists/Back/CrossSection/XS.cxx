...
void NormalizeByFluxAndTargets( MnvH2D *&h_normalized, MnvH2D *h_effcor, MnvH1D* h_flux, double pot_scale, bool applyFluxConstraint, NukeCCUtilsNSF* utils, TMatrixD unfoldCov, MnvH2D *flux_norm = NULL)
{
...
*/
  ...
  if(mytitle.find("pzmu_ptmu_data")!=string::npos) finalCovMatrix.SaveAs("CovMat_tocheck_pzmu.root");
  if(mytitle.find("enu_q2_data")!=string::npos) finalCovMatrix.SaveAs("CovMat_tocheck_enu.root");
  if(mytitle.find("q2_ptmu_data")!=string::npos) finalCovMatrix.SaveAs("CovMat_tocheck_q2.root");

  TCanvas c1("can","can");
  finalCovMatrix.Draw("COLZ");
  if(mytitle.find("pzmu_ptmu_data")!=string::npos) c1.Print("CovMat_tocheck_pzmu_noNanto3.png");
  if(mytitle.find("enu_ptmu_data")!=string::npos) c1.Print("CovMat_tocheck_enu_noNanto3.png");
...
int CrossSectionHists(std::string event_filename,std::string scale_filename,std::string migration_filename,std::string efficiency_filename,std::string output_filename,int num_iter,int ana_type ){
  ...
  NukeCCUtilsNSF *utils = new NukeCCUtilsNSF("minervame5A"); //you can give any name you want to give here by the way....
 ...
 FluxReweighter *frw = new FluxReweighter(-14,false,FluxReweighter::minervame5A,FluxReweighter::gen2thin, FluxReweighter::g4numiv6, 100); 
   ...
   const std::string hist_type[2]  = {"h_pzmu_ptmu_","h_enu_q2_"};
   const std::string truth_type[2]  = {"h_truth_pzmu_ptmu_","h_truth_enu_q2_"};
   
   const std::string migration_type[2] = {"h_pzmu_ptmu_","h_q2_enu_"};
...
//#endif    
  ...
  MnvH2D* h_hist_type_generated = (MnvH2D*)migration_file->Get((migration_type[ana_type]+"qelike_truth").c_str());
  MnvH2D *h_hist_type_migration = (MnvH2D*)migration_file->Get((migration_type[ana_type]+"qelike_migration").c_str());
  ...
  migstatDummy->SaveAs("h_pzmu_ptmu_migration.root");    
 ...
 std::cout<<" I Am HERE 3"<<std::endl;  
   ...
   //Get THE POT NORMALIZATOIN
   double pot_data = pot_event->X();
   double pot_mc = pot_event->Y();
   
   double pot_scale = pot_data/pot_mc;
  ...
  // h_hist_type_migration->Scale(pot_migration_scale);
   ...
   MnvH2D *h_mc_no_bck = signal_dict["qelike"]->Clone((hist_type[ana_type]+"qelikebkgsub").c_str());
  ...
  // h_mc_no_bck->Add(bkgsub_hist["scaled_bkg"],-1.0);
   ...
   h_mc_unfolded->SetName((hist_type[ana_type]+"mc_unfolded").c_str());

   MnvH2D *h_data_unfolded = new MnvH2D(*h_mc_unfolded);
   h_data_unfolded->SetName((hist_type[ana_type]+"data_unfolded").c_str());
 ...
 // There's a bug in RooUnfold that's making it return covariance
  ...
  MnvH2D *h_hist_type_effhist_num = (MnvH2D*)eff_file->Get((hist_type[ana_type]+"qelike").c_str());
  MnvH2D *h_hist_type_effhist_den = (MnvH2D*)eff_file->Get((truth_type[ana_type]+"qelike").c_str());
...
//  h_hist_type_effhist_den->Scale(pot_eff_scale);
  ...
  CorrectByEfficiency(h_hist_type_mc_effcor,h_mc_unfolded,h_hist_type_effhist_num,h_hist_type_effhist_den);
   ...
   std::string effcorname = ana_type==0?"h_effcor_pzmu_ptmu_":"h_effcor_enu_q2_";
  ...
  test.Print();
   ...
   if(ana_type==1)h_flux_normalization = GetEnuFluxHists(h_hist_type_effhist_den,h_flux);
    ...
    //Get the Truth distribution (the efficiency denominator)......
      MnvH2D *h_hist_type_truthcomp = (MnvH2D*)eff_file->Get((truth_type[ana_type]+components[i]).c_str());
    ...
    if(ana_type==0)NormalizeByFluxAndTargets( h_hist_type_cross_section_comp, h_hist_type_truthcomp, h_flux, pot_data, applyFluxConstraint, utils, unfoldingCovMatrixEff_hist_type);
    
     else{
     NormalizeByFluxAndTargets( h_hist_type_cross_section_comp, h_hist_type_truthcomp, h_flux, pot_data, applyFluxConstraint, utils, unfoldingCovMatrixEff_hist_type, h_flux_normalization);
  ...
  std::cout << "TRACE: make and fill the cross section histograms " << std::endl;
    ...
    if(ana_type==0){  
    NormalizeByFluxAndTargets( h_hist_type_cross_section_data, h_hist_type_data_effcor, h_flux, pot_data, applyFluxConstraint, utils, unfoldingCovMatrixEff_hist_type);
    NormalizeByFluxAndTargets( h_hist_type_cross_section_mc, h_hist_type_mc_effcor, h_flux, pot_data, applyFluxConstraint, utils, unfoldingCovMatrixEff_hist_type);
    
    }
    
    if(ana_type==1){
    
    NormalizeByFluxAndTargets( h_hist_type_cross_section_data, h_hist_type_data_effcor, h_flux, pot_data, applyFluxConstraint, utils, unfoldingCovMatrixEff_hist_type,h_flux_normalization);
    NormalizeByFluxAndTargets( h_hist_type_cross_section_mc, h_hist_type_mc_effcor, h_flux, pot_data, applyFluxConstraint, utils, unfoldingCovMatrixEff_hist_type,h_flux_normalization);
...
//   frw->CheckAndFixFluxErrorBand(h_hist_type_cross_section_data);
   ...
   mc_component->Write((hist_type[ana_type]+"mc").c_str());
   data_component->Write((hist_type[ana_type]+"data").c_str());
   ...
   h_hist_type_cross_section_data->Write((hist_type[ana_type]+"dataCrossSection").c_str());
   h_hist_type_cross_section_mc->Write((hist_type[ana_type]+"qelike_recoCrossSection").c_str());
   pot_event->Write("pot");
#ifndef NEWCOV
...
int main( int argc, char *argv[])
{
  ...
  return CrossSectionHists(par[1], par[2],par[3],par[4],par[5],atoi(par[6].c_str()),atoi(par[7].c_str()));
