...
void makePlots(bool doMultipliers, bool doTracks, bool doRatio, string location, string varstring,string xvar, string yvar)
{
  ...
  //need pzmuptmu
  MnvH2D* mcMnv=(MnvH2D*)f3.Get(Form("h_%s_mc",varstring.c_str()));//Get from N track
  MnvH2D* dataMnv = (MnvH2D*)f3.Get(Form("h_%s_data",varstring.c_str()));//Get from N track
  MnvH2D* SingleTrackMnv = (MnvH2D*)f1.Get(Form("h_%s_data",varstring.c_str()));//Get from 1 track
  MnvH2D* MultiTrackMnv = (MnvH2D*)f2.Get(Form("h_%s_data",varstring.c_str()));//Get from 2+ track
  MnvH2D* SingleTrackMCMnv = (MnvH2D*)f1.Get(Form("h_%s_mc",varstring.c_str()));//Get from 1 track
  MnvH2D* MultiTrackMCMnv = (MnvH2D*)f2.Get(Form("h_%s_mc",varstring.c_str()));//Get from 2+ track
  ...
  MnvH2D* mcMnv_qelikenot =  (MnvH2D*)f3.Get(Form("h_%s_qelikenot",varstring.c_str()));//Get from N track
  
  cout << "Got histograms" << endl;
  //  MnvH2D* mcMnv_qelike_2p2h_no_lowrec = (MnvH2D*)f4.Get("h_%s_qelike_oth");//Get from N track
  // //bkgs
  // MnvH2D* mcMnv_1track_bkg = (MnvH2D*)f1.Get("h_%s_qelikenot");//Get from 1 track
  // MnvH2D* mcMnv_2track_bkg = (MnvH2D*)f2.Get("h_%s_qelikenot");//Get from 2+ track
  // //the constraint
  ...
  cout << dataMnv << endl;  
  dataMnv->GetXaxis()->SetTitle("x axis");
  ...
  cout << "here" << endl;
  SingleTrackMnv->Scale(1e-5, "width");
  MultiTrackMnv->Scale(1e-5, "width");
  SingleTrackMCMnv->Scale(1e-5, "width");
  MultiTrackMCMnv->Scale(1e-5, "width");
  ...
  //do by physics process
    ...
    histAndOpts.push_back(std::make_pair(mc_qelikenot,       "histl"));
  }
  ...
  if(doRatio){
    ...
    gc->SetYTitle("Ratio to MnvGENIEv1");
  }
  else{
    gc->SetYLimits(0, 75);
  ...
  if(doRatio){
    ...
    gc2->SetYTitle("Ratio to MnvGENIEv1");
  }
  else{
    gc2->SetYLimits(0, 75);
...
int main(int argc, char* argv[])
{
  ...
  string yvar = argv[4];
  //multipliers
  makePlots(true,true,false,location,varstring,xvar,yvar);
  makePlots(true,false,false,location,varstring,xvar,yvar);
  //standard
  makePlots(false,true,false,location,varstring,xvar,yvar);
  makePlots(false,false,false,location,varstring,xvar,yvar);
  makePlots(false,false,true,location,varstring,xvar,yvar);
