 ...
 if(argc==1){
  ...
  //GetErrorBands(util.m_mc);
   ...
   for (auto v : variables2DMC) {
               ...
               v->m_selected_mc_reco_true_target.SyncCVHistos();
               v->m_selected_mc_reco_USCH.SyncCVHistos();
               v->m_selected_mc_reco_DSCH.SyncCVHistos();
               v->m_selected_mc_reco_other.SyncCVHistos();
               v->m_selected_trans_SB.SyncCVHistos();
               v->m_selected_contin_SB.SyncCVHistos();
   }
...
void FillVariable( PlotUtils::ChainWrapper* chain, HelicityType::t_HelicityType helicity, NukeCCUtilsNSF *utils , NukeCC_Cuts *cutter ,NukeCC_Binning  *binsDef ,std::vector<Var*>& variables,std::vector<Var2D*>& variables2d,bool isMC,int targetID, int targetZ, const string playlist, bool doDIS){
  ...
  if (doDIS){
   ...
   variables = {emu, ehad, enu, x, y, planeDNN};//{enu,ehad}; 
	 ...
	 reco4++;
	     ...
	     if( !cutter->PassDISCut( universe ))continue;
           ...
           if(cutter->IsInTrueMaterial(universe,targetID,targetZ,false) ){ 
               if (cutter->PassTrueDISCut( universe ) && cutter->PassTrueMuEnergyCut(universe) && cutter->PassTrueThetaCut(universe)  && cutter->PassTrueFiducial(universe)) 
                    v->m_selected_mc_reco_true_target.univHist(universe)->Fill(v->GetRecoValueX(*universe), v->GetRecoValueY(*universe), universe->GetWeight());
            }
           else{
                if (region ==3 )
                  v->m_selected_mc_reco_USCH.univHist(universe)->Fill(v->GetRecoValueX(*universe), v->GetRecoValueY(*universe), universe->GetWeight());
                else if (region ==4 )
                  v->m_selected_mc_reco_DSCH.univHist(universe)->Fill(v->GetRecoValueX(*universe), v->GetRecoValueY(*universe), universe->GetWeight());
                else
          ...
          if ( cutter->IsInTrueMaterial(universe,targetID,targetZ,false)){
             // if (cutter->PassTrueDISCut( universe )) {
                        if(sideBand==0) v->m_selected_trans_SB.univHist(universe)->Fill(v->GetRecoValueX(*universe), v->GetRecoValueY(*universe), universe->GetWeight());
                        if(sideBand==1) v->m_selected_contin_SB.univHist(universe)->Fill(v->GetRecoValueX(*universe), v->GetRecoValueY(*universe), universe->GetWeight());
        }         
	   }//close 2D loop here
	  ...
	  for (auto v : variables){
	     ...
	     v->m_selected_mc_reco.univHist(universe)->Fill(v->GetRecoValue(*universe, 0), universe->GetWeight()); 
           if (targetID < 10){
             //if(cutter->IsInTrueMaterial(universe,targetID,targetZ,false) && cutter->PassTrueDISCut( universe ) && cutter->PassTrueMuEnergyCut(universe) && cutter->PassTrueThetaCut(universe)  && cutter->PassTrueFiducial(universe) ){ 
           if(cutter->IsInTrueMaterial(universe,targetID,targetZ,false) ){ 
               if (cutter->PassTrueDISCut( universe ) && cutter->PassTrueMuEnergyCut(universe) && cutter->PassTrueThetaCut(universe)  && cutter->PassTrueFiducial(universe)) 
