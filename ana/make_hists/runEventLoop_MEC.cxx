...
//========================This part is added by Oscar to adapt=====================
   //bool value = passesAll(universe);
   /*
   bool cutBasic =cutter->passesAll(universe);
   //if(!cutBasic) continue;


   double q3 = universe->Getq3Reco();
   double energy = universe->GetECALHCALAvEnergy();

   double weight=1;
   double rpaWeight=1;
   double maccqeWeight=1;

   double q3_truth = universe->Getq3Truth();
   double energy_true = universe->GetEavailTrue();
   double q0_truth = universe->Getq0Truth();

   maccqeWeight = universe->GetVecElem("truth_genie_wgt_MaCCQE",2);

   double cvWeight = universe->GetCVWeight();
   weight*=cvWeight;
   double wgt_2p2h = universe->GetLowRecoil2p2hWeight();
   double wgt_rpa = universe->GetRPAWeight();
   weight *= universe->GetWeightEmu();

*/
//========================End of part added by Oscar to adapt=====================
	 reco1++;
  ...
  //std::cout<<"emu: "<<emu<<endl;
	 ...
	 int mc_type;
        ...
        /*  int sideBand = -1;
	     ...
	     //if( !cutter->PassDISCut( universe ))continue;
             ...
             //std::cout<<" size of vector: "<< FateNumV.size() << std::endl;
                for(int i=0;i<FateNumV.size();i++){
                  ...
                  if(preFSIV[i] == 111 || preFSIV[i]==211 || preFSIV[i] ==-211 ){
                      //pionWeight = universe->getFateTable(preFSIEn[i], FateNumV[i]);
		                  pionWeight = 1.0;
                    }
		  ...
		  //std::cout<<"Weight here is: "<<weight<<std::endl;
                  double Tweight= universe->GetTruthWeight();
		  //std::cout<<"Truth Weight here is: "<<Tweight<<std::endl;
                  ...
                  if(FateNumV[i] == 1){v->m_selected_mc_reco_NoScattering.univHist(universe)->Fill(v->GetRecoValueX(*universe), v->GetRecoValueY(*universe),FinalWeight );
   ...
   //std::cout<<"Weight: "<< weight <<" pion weight: "<<PionFateWeight <<" final weight: "<< weight*PionFateWeight<<" Fate: "<<FateNumV[i] << " preFSI: "<<preFSIV[i] <<" PionEnergy: "<<PionEnergy <<std::endl;
                  ...
                  if(FateNumV[i] == 2){v->m_selected_mc_reco_ChargeExt.univHist(universe)->Fill(v->GetRecoValueX(*universe), v->GetRecoValueY(*universe), FinalWeight );
                                       ...
                                       hist_fate_ChEx->Fill(preFSIEn[i],TFinalWeight);} //,TFinalWeight
                  if(FateNumV[i] == 3){v->m_selected_mc_reco_Elasticity.univHist(universe)->Fill(v->GetRecoValueX(*universe), v->GetRecoValueY(*universe),FinalWeight  );
                  ...
                  if(FateNumV[i] == 5){v->m_selected_mc_reco_Absorption.univHist(universe)->Fill(v->GetRecoValueX(*universe), v->GetRecoValueY(*universe),FinalWeight  );
                  ...
                  if(FateNumV[i] == 50){v->m_selected_mc_reco_fate_nn_50.univHist(universe)->Fill(v->GetRecoValueX(*universe), v->GetRecoValueY(*universe),FinalWeight  );
                  ...
                  if(FateNumV[i] == 51){v->m_selected_mc_reco_fate_pn_51.univHist(universe)->Fill(v->GetRecoValueX(*universe), v->GetRecoValueY(*universe),FinalWeight  );
                  ...
                  if(FateNumV[i] == 52){v->m_selected_mc_reco_fate_pp_52.univHist(universe)->Fill(v->GetRecoValueX(*universe), v->GetRecoValueY(*universe),FinalWeight  );
                  ...
                  if(FateNumV[i] == 8){v->m_selected_mc_reco_PionProd.univHist(universe)->Fill(v->GetRecoValueX(*universe), v->GetRecoValueY(*universe), FinalWeight );
                  ...
                  if(FateNumV[i] == 9){v->m_selected_mc_reco_MultNuc.univHist(universe)->Fill(v->GetRecoValueX(*universe), v->GetRecoValueY(*universe), FinalWeight );
                  ...
                  if(FateNumV[i] == 10){v->m_selected_mc_reco_KnockOut.univHist(universe)->Fill(v->GetRecoValueX(*universe), v->GetRecoValueY(*universe), FinalWeight );
...
/*
             ...
             if (maxElement != preFSIEn.end()) {
               ...
               index = std::distance(preFSIEn.begin(), maxElement);
             ...
             double TFinalWeight = Tweight*pionWeight;
                  ...
                  //if(PionFateWeight != 1){
                  //std::cout<<"Weight: "<< weight <<" pion weight: "<<PionFateWeight <<" final weight: "<< weight*PionFateWeight<<" Fate: "<<FateNumV[i] << " preFSI: "<<preFSIV[i] <<" PionEnergy: "<<PionEnergy <<std::endl;
                  if(FateNumV[index] == 1){v->m_selected_mc_reco_NoScattering.univHist(universe)->Fill(v->GetRecoValueX(*universe), v->GetRecoValueY(*universe),FinalWeight );
                                       v->m_selected_mc_truth_reco_NoScattering.univHist(universe)->Fill(v->GetTrueValueX(*universe), v->GetTrueValueY(*universe),TFinalWeight );
   //std::cout<<"Weight: "<< weight <<" pion weight: "<<PionFateWeight <<" final weight: "<< weight*PionFateWeight<<" Fate: "<<FateNumV[i] << " preFSI: "<<preFSIV[i] <<" PionEnergy: "<<PionEnergy <<std::endl;
                                     }
                  if(FateNumV[index] == 2){v->m_selected_mc_reco_ChargeExt.univHist(universe)->Fill(v->GetRecoValueX(*universe), v->GetRecoValueY(*universe), FinalWeight );
                                       v->m_selected_mc_truth_reco_ChargeExt.univHist(universe)->Fill(v->GetTrueValueX(*universe), v->GetTrueValueY(*universe),TFinalWeight  );}
                  if(FateNumV[index] == 3){v->m_selected_mc_reco_Elasticity.univHist(universe)->Fill(v->GetRecoValueX(*universe), v->GetRecoValueY(*universe),FinalWeight*0.5  );
                  ...
                  if(FateNumV[index] == 5){v->m_selected_mc_reco_Absorption.univHist(universe)->Fill(v->GetRecoValueX(*universe), v->GetRecoValueY(*universe),FinalWeight  );
                                       v->m_selected_mc_truth_reco_Absorption.univHist(universe)->Fill(v->GetTrueValueX(*universe), v->GetTrueValueY(*universe),TFinalWeight  );}
                  if(FateNumV[index] == 8){v->m_selected_mc_reco_PionProd.univHist(universe)->Fill(v->GetRecoValueX(*universe), v->GetRecoValueY(*universe), FinalWeight );
                                       v->m_selected_mc_truth_reco_PionProd.univHist(universe)->Fill(v->GetTrueValueX(*universe), v->GetTrueValueY(*universe), TFinalWeight );}
                  if(FateNumV[index] == 9){v->m_selected_mc_reco_MultNuc.univHist(universe)->Fill(v->GetRecoValueX(*universe), v->GetRecoValueY(*universe), FinalWeight );
                                       v->m_selected_mc_truth_reco_MultNuc.univHist(universe)->Fill(v->GetTrueValueX(*universe), v->GetTrueValueY(*universe),TFinalWeight  );}
                  if(FateNumV[index] == 10){v->m_selected_mc_reco_KnockOut.univHist(universe)->Fill(v->GetRecoValueX(*universe), v->GetRecoValueY(*universe), FinalWeight );
                                        v->m_selected_mc_truth_reco_KnockOut.univHist(universe)->Fill(v->GetTrueValueX(*universe), v->GetTrueValueY(*universe), TFinalWeight );}
