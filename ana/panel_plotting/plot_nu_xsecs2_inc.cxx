...
void makePlots(bool doMultipliers,bool doGenies,string location)
{
  ...
  TFile f1(Form("%s_CV_RPA_Res_Nieves/CrossSection_per_nucleon_iterations_10_CombinedPlaylists.root_pzmu.root",location.c_str()));//Final result
  //  TFile f2(Form("%s_default/CrossSection_per_nucleon_iterations_10_CombinedPlaylists.root_pzmu.root",location.c_str()));//Default GENIE
  //  TFile f3(Form("%s_pion_rpa/CrossSection_per_nucleon_iterations_10_CombinedPlaylists.root_pzmu.root",location.c_str()));//GENIE+2p2h+RPA (aka no tune)
  ...
  //  MnvH2D* mcMnv_2p2h_no_lowrec = (MnvH2D*)f3.Get("h_pzmu_ptmu_cross_section_2p2h");//Get from N track
  ...
  //  mcMnv_2p2h_no_lowrec->Scale(1e39,"width");
  ...
  mc_res->SetLineColor(mycolors[4]);
  mc_dis->SetLineColor(mycolors[5]);
  mc_2p2h->SetLineColor(mycolors[6]);
  //  mc_2p2h_no_lowrec->SetLineColor(mycolors[6]);
  //  mc_2p2h_no_lowrec->SetLineStyle(3);
  ...
  //  }
  //  else{
    histAndOpts.push_back(std::make_pair(mc_qe,       "hist l"));
    histAndOpts.push_back(std::make_pair(mc_res,       "hist l"));
    histAndOpts.push_back(std::make_pair(mc_dis,       "hist l"));
    histAndOpts.push_back(std::make_pair(mc_2p2h,       "hist l"));
    //    histAndOpts.push_back(std::make_pair(mc_2p2h_no_lowrec,       "hist l"));
    //  }
  ...
  //  else{
    ...
    //    leg->AddEntry(mc_2p2h_no_lowrec,"2p2h without fit","l");
    //  }
  leg->Draw("SAME");
  //  if(doGenies){
  ...
  // Values to multiply each bin by to get them on a similar range
  double  multipliers2[]={30, 8, 3, 2, 1,
			  1, 1, 1, 2, 4,
                          20, 100, 1000};
