...
void makePlots(bool doMultipliers,string location,string varx, string vary, bool doRatio, string sample, int projx_column=-1, int projx_row=-1, int projx_pixelx=-1, int projx_pixely=-1, int projy_column=-1, int projy_row=-1, int projy_pixelx=-1, int projy_pixely=-1)
{
  ...
  string filename = Form("%s_CV/CrossSection_CombinedPlaylists.root_%s.root",location.c_str(),varspecfilename.c_str());//Final result
  string filename_noenh = Form("%s_pion_rpa/CrossSection_CombinedPlaylists.root_%s.root",location.c_str(),varspecfilename.c_str());//Final result
  if(vary!="ptmu"){
    filename = Form("%s_CV/CrossSection_%s_CombinedPlaylists.root",location.c_str(),varspecfilename.c_str());//Final result
    filename_noenh = Form("%s_pion_rpa/CrossSection_%s_CombinedPlaylists.root",location.c_str(),varspecfilename.c_str());//Final result
  }
  TFile f1(filename.c_str());
  TFile f2(filename_noenh.c_str());
  ...
  MnvH2D* mcMnv_noenh=(MnvH2D*)f2.Get(Form("h_%s_mc_nobck_unfold_effcor_cross_section",varspec.c_str()));
  ...
  MnvH2D* mcMnv_qelike_2p2h_noenh = (MnvH2D*)f2.Get(Form("h_%s_cross_section_qelike_2p2h",varspec.c_str()));//Get from N track

  dataMnv->Scale(1e39, "width");
  mcMnv->Scale(1e39, "width");
  mcMnv_noenh->Scale(1e39, "width");
  ...
  mcMnv_qelike_2p2h_noenh->Scale(1e39,"width");
  ...
  TH2* mc_noenh=new TH2D(mcMnv_noenh->GetCVHistoWithStatError());
  ...
  TH2* mc_qelike_2p2h_noenh = new TH2D(mcMnv_qelike_2p2h_noenh->GetCVHistoWithStatError());

  TH2* mc_enh_only = (TH2*)mc->Clone("mc_enh_only");
  mc_enh_only->Add(mc_noenh,-1);
  ...
  if(doRatio){
    ...
    mc_qelike_2p2h_noenh->Divide(mc);
    mc_noenh->Divide(mc);
    mc_enh_only->Divide(mc);
    mc->Divide(mc);
  }
  ...
  mc_noenh->SetLineColor(kRed);
  mc_noenh->SetLineWidth(2);
  mc_noenh->SetLineStyle(3);
  
  mc_enh_only->SetLineColor(kViolet);
  ...
  mc_qelike_2p2h_noenh->SetLineColor(mycolors[16]);
  ...
  mc_qelike_2p2h_noenh->SetLineStyle(3);
  ...
  if(sample=="Standard"){
    ...
    histAndOpts.push_back(std::make_pair(mc_qelike_2p2h_noenh , "hist l"));
    histAndOpts.push_back(std::make_pair(mc_noenh , "hist l"));
    histAndOpts.push_back(std::make_pair(mc_enh_only , "hist l"));
  }
  else if(sample=="QE"){
    histAndOpts.push_back(std::make_pair(mc_qelike_qe,       "hist l"));
  ...
  if(projx_column==-1){
    ...
    gc= plotYAxis1D_ReducedXRange(histAndOpts, vary.c_str() ,varx.c_str(),projy_column,projy_row,0.0,1.0,projy_pixelx,projy_pixely ,doMultipliers ? &multi_1[0] : NULL);
  }
  ...
  TLegend* leg=new TLegend(0, 0.94, 1.0, 1);
  leg->SetNColumns(5);
  leg->SetFillStyle(0);
  leg->SetBorderSize(0);
  ...
  if(sample=="Standard"){
    ...
    leg->AddEntry(mc_qelike_2p2h_noenh,"2p2h w/o enhancement","l");
    leg->AddEntry(mc_noenh,"w/o enhancement","l");
    leg->AddEntry(mc_enh_only,"2p2h enhancement","l");
  }
  else if(sample=="QE"){
    leg->AddEntry(mc_qelike_qe,"QE","l");
