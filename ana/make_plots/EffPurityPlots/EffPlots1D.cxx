...
void makeplot(string histsuffix, TFile* myfile, bool doprelimlabel=false){
  ...
  PlotUtils::MnvH1D *num = (PlotUtils::MnvH1D*)myfile->Get("h_mc_Emu");
  PlotUtils::MnvH1D *dem = (PlotUtils::MnvH1D*)myfile->Get("h_truth_Emu");
  ...
  num->GetXaxis()->SetTitle("Muon Energy (GeV)");
  ...
  //num->GetXaxis()->SetRangeUser(3,10);
  //num->GetYaxis()->SetRangeUser(3,10);
...
------"<<std::endl;
  ...
  TFile *infile = new TFile(Form("%s/Hists_Efficiency_t%d_z%d_AntiNu_minervame6B.root",location.c_str(),targetID,targetZ));
