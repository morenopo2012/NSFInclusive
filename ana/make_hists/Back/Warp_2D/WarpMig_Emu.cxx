     ...
     //=========================================
	   ...
	   if(!cutter->IsInTrueMaterial(universe,targetID, targetZ,false)) continue;
	     ...
	     //double wgt = universe->GetWeight();
             ...
             v->m_selected_mc_reco.univHist(universe)->Fill(v->GetRecoValueX(*universe), v->GetRecoValueY(*universe), universe->GetWeightEmu()); 
             //v->m_selected_mc_reco.univHist(universe)->Fill(v->GetRecoValueX(*universe), v->GetRecoValueY(*universe), wgt); 
             //v->m_selected_Migration.univHist(universe)->Fill(v->GetRecoValueX(*universe), v->GetTrueValueX(*universe), universe->GetWeightEmu()); 
             v->m_selected_Migration.univHist(universe)->Fill(v->GetTrueValueX(*universe), v->GetTrueValueY(*universe), universe->GetWeightEmu()); 
     //Migration stuff
             v->FillResponse(v->GetRecoValueX(*universe), v->GetRecoValueY(*universe),v->GetTrueValueX(*universe), v->GetTrueValueY(*universe),universe->ShortName(),universe->GetWeightEmu(),unv_count); 
		 }
	   ...
	   for (auto v : variables){
	     ...
	     //Added for 1D analysis 
             v->m_selected_mc_reco.univHist(universe)->Fill(v->GetRecoValue(*universe, 0), universe->GetWeightEmu());
	     v->m_selected_mc_true.univHist(universe)->Fill(v->GetTrueValue(*universe, 0), universe->GetWeightEmu());
	    //////////////Migration//////////////////
              v->FillResponse1D(v->GetRecoValue(*universe),v->GetTrueValue(*universe),universe->ShortName(),universe->GetWeightEmu(),unv_count); 
	   	 }
		 unv_count++;
...
int main(int argc, char *argv[]){
	 ...
	 TString histFileName = utils->GetHistFileName( "MigrationEmuEhad", FileType::kAny, targetID, targetZ, helicity ); 
