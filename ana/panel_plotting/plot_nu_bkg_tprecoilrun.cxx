...
void Create3DBkgConst(MnvH2D*&h, MnvH2D* h_temp, HyperDimLinearizer *hd, vector<double> zcoords){
  ...
  for(uint z=0;z<zcen.size();z++){//loop over Z
	...
	if(scale==0) scale=1;
	vector<double> vals;
	vals.push_back(h_temp->GetXaxis()->GetBinCenter(i)/1000.);
	vals.push_back(h_temp->GetYaxis()->GetBinCenter(j)/1000.);
  ...
  for(uint v=0;v<verrors.size();v++){
	...
	for(int i=0;i<num_recoil_bins;i++){//recoil
	  for(int j=0;j<num_pt_bins;j++){//pt
	    ...
	    vals.push_back(h_vband_temp->GetYaxis()->GetBinCenter(j)/1000.0);
	    vals.push_back(zcen[z]);
...
void makePlots(bool doMultipliers, string location,string sample, bool doRatio, bool constrainSig)
{
  ...
  TFile f3(Form("/pnfs/minerva/persistent/users/drut1186/3D_ME_LongPaper/PubCandidate_v2_withLateralSys/CCQELike_CV_Zexp_Bubble_PtRecoil/MuonEventSelection_MakeFlux-1_Multiplicity-0_Sample-%s_CombinedPlaylists.root",sample.c_str()));//sample with the pt_eavail applied as an event weight
  ...
  MnvH2D* dataMnv_1trk=(MnvH2D*)f2.Get("h_enuproxyE_data");
  //  MnvH2D* dataMnv_2trk=(MnvH2D*)f3.Get("h_enuproxyE_data");
  
  MnvH2D* mcsigMnv_1trk=(MnvH2D*)f2.Get("h_enuproxyE_qelike");
  //  MnvH2D* mcsigMnv_2trk=(MnvH2D*)f3.Get("h_enuproxyE_qelike");

  MnvH2D* mcbkgMnv_1trk=(MnvH2D*)f2.Get("h_enuproxyE_qelikenot");
  //  MnvH2D* mcbkgMnv_2trk=(MnvH2D*)f3.Get("h_enuproxyE_qelikenot");

  MnvH2D* eventreweighted=(MnvH2D*)f3.Get("h_enuproxyE_mc");

  /*
  MnvH2D* mcbkgMnv_scp_1trk=(MnvH2D*)f2.Get("h_enuproxyE_qelikenot_singlechargedpion");
  MnvH2D* mcbkgMnv_scp_2trk=(MnvH2D*)f3.Get("h_enuproxyE_qelikenot_singlechargedpion");

  MnvH2D* mcbkgMnv_snp_1trk=(MnvH2D*)f2.Get("h_enuproxyE_qelikenot_singleneutralpion");
  MnvH2D* mcbkgMnv_snp_2trk=(MnvH2D*)f3.Get("h_enuproxyE_qelikenot_singleneutralpion");

  MnvH2D* mcbkgMnv_mp_1trk=(MnvH2D*)f2.Get("h_enuproxyE_qelikenot_multipion");
  MnvH2D* mcbkgMnv_mp_2trk=(MnvH2D*)f3.Get("h_enuproxyE_qelikenot_multipion");

  MnvH2D* mcbkgMnv_nop_1trk=(MnvH2D*)f2.Get("h_enuproxyE_qelikenot_not_scp_snp_mp");
  MnvH2D* mcbkgMnv_nop_2trk=(MnvH2D*)f3.Get("h_enuproxyE_qelikenot_not_scp_snp_mp");
  */
  //unscaled
  MnvH2D* mcMnv_1trk=(MnvH2D*)f2.Get("h_enuproxyE_mc");
  //  MnvH2D* mcMnv_2trk=(MnvH2D*)f3.Get("h_enuproxyE_mc");
  ...
  */
  ...
  mcsigMnv_1trk->Write("h_enuproxyE_mc_1trk");
  //  mcsigMnv_2trk->Write("h_enuproxyE_mc_2trk");
  mcsigMnv_alltrk->Write("h_enuproxyE_mc_alltrk");
  dataMnv_1trk->Write("h_enuproxyE_data_1trk");
  //  dataMnv_2trk->Write("h_enuproxyE_data_2trk");
  dataMnv_alltrk->Write("h_enuproxyE_data_alltrk");
  mcbkgMnv_1trk->Write("h_enuproxyE_mc_1trk_bkgfraction");
  //  mcbkgMnv_2trk->Write("h_enuproxyE_mc_2trk_bkgfraction");
  mcMnv_alltrk->Write("h_enuproxyE_mc_alltrk_unconstrained");
  ...
  */
  ...
  cout << "DONE" << endl;
  std::vector<TH2D*> reweightedresults = my3d->Get2DHistos(eventreweighted,false);
  cout << "DOUBLE DONE" << endl;
  ...
  for(unsigned int i=1;i<pz3Dbins.size();i++){
    ...
    */
    ...
    reweightedresults[i]->Scale(1/pzwidth,"width");
  }
  ...
  for(unsigned int i=1;i<pz3Dbins.size();i++){
    ...
    */
    ...
    reweightedresults[i]->SetLineStyle(2);
    reweightedresults[i]->SetLineWidth(3);
    /*
    if(true){
      for(int j=1;j<7;j++){
    ...
    */
	...
	for(int j=0;j<ybins;j++){
      ...
      */
      ...
      reweightedresults[i]->Divide(tmp_alltrk);
    }      
    ...
    */
    ...
    histAndOpts_alltrk.push_back(std::make_pair(reweightedresults[i], "histL"));
    ...
    leg->AddEntry(reweightedresults[i], "Minerva Tune v1 with pt-E_{available} bkg tune","l");
    

    TLatex mytex;
    mytex.SetTextSize(0.05);
    string mystring =     Form("%.2f < E_{#mu} (GeV) < %.2f",pz3Dbins[i-1],pz3Dbins[i]);
    /*
    GridCanvas* gc1=NULL;
    ...
    */
    ...
    GridCanvas* gc3=NULL;
    gc3=plotXAxis1D(histAndOpts_alltrk, "Visible Energy (GeV)","q_{0}^{QE}", doMultipliers ?  &multi_3[0] : NULL);
    ...
    else gc3->SetYTitle("Events per MeV#timesGeV^{2}");
    leg->Draw("SAME");
    gc3->Modified();
