...
void makePlots(bool doMultipliers)
{
  ...
  //three files 1-track, 2+track, N-track
   ...
   TFile f1 ("/minerva/app/users/zdar/cmtuser/Minerva_v22r1p1_MADNew/Ana/NSFNukeCCInclusive/ana/make_hists/CrossSection_NSF/XS_pzmu_t3_z26.root");
   //TFile f1 ("/minerva/app/users/zdar/cmtuser/Minerva_v22r1p1_MADNew/Ana/NSFNukeCCInclusive/ana/make_hists/CrossSection_NSF/XS_pzmu.root");

  //need pzmuptmu
  MnvH2D* mcMnv=(MnvH2D*)f1.Get("h_effcor_Emu_Ehad_mc");
  MnvH2D* dataMnv = (MnvH2D*)f1.Get("h_effcor_Emu_Ehad_data");
  ...
  //gc->SetYLimits(0, 15);
  //gc->SetYLimits(0, 200);
  gc->SetYTitle("Efficiency Corr event rate(x10^{-3}) per GeV^{2}");
  ...
  leg->AddEntry(" ","Iron of target 3");
  //leg->AddEntry(" ","Lead of Target 3");
  //leg->AddEntry(" ","Lead of Target 3");
  leg->Draw("SAME");
    gc->Print(doMultipliers ? "nu-2d-effCorrected-NT-model-Ehad-multiplier.png" : "nu-2d-effCorrected-NT-model-Ehad.png");
  ...
  // Values to multiply each bin by to get them on a similar range
  double  multipliers4[]={1.5, 1, 1, 1.5, 3, 10, 20, 40, 70, 100,
			  200, 500, 1};
  ...
  gc2->SetYLimits(0, 1.5);
  //gc2->SetYLimits(0, 90);
  //gc2->SetYLimits(0, 15);
  //gc2->SetYLimits(0, 200);
  //gc2->Remax();
  gc2->SetYTitle("Efficiency Corr event rate(x10^{-3}) per GeV^{2}");
  //gc2->SetYTitle("Event Rate");
  gc2->Modified();
    

  gc2->Print(doMultipliers ? "nu-2d-effCorrected-NT-model-Emu-multiplier.png" : "nu-2d-effCorrected-NT-model-Emu.png");
