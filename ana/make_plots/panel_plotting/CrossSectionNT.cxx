...
void makePlots(bool doMultipliers)
{
  ...
  TFile f1("/minerva/app/users/zdar/cmtuser/Minerva_v22r1p1_MADNew/Ana/NSFNukeCCInclusive/ana/make_hists/CrossSection_NSF/XS_pzmu_t3_z26.root");//1track
  ...
  dataMnv->Scale(1e+39, "width");
  mcMnv->Scale(1e+39, "width");
  mcMnv->Scale(0.097);
  ...
  GridCanvas* gc=plotXAxis1D(histAndOpts, "Muon Energy (GeV)", "Ehad", doMultipliers ? multipliers0 : NULL);
  ...
  gc->SetYTitle("d#sigma^{DIS}/dE_{#mu}dE_{#recoil} (x10^{-39}) cm^{2} per GeV^{2} per Nucl.");
  ...
  //leg->AddEntry(" ","Lead of Target 3");
  leg->Draw("SAME");
    gc->Print(doMultipliers ? "nu-2d-CrossSection-NT-model-Ehad-multiplier.png" : "nu-2d-CrossSection-NT-model-Ehad.png");
  ...
  // Values to multiply each bin by to get them on a similar range
  double  multipliers4[]={2, 1, 1, 1.5, 5, 20, 30, 50, 90, 200,
			  400, 1000, 1};
  ...
  gc2->SetYTitle("d#sigma^{DIS}/dE_{#mu}dE_{#recoil} (x10^{-39}) cm^{2} per GeV^{2} per Nucleons");
  //gc2->SetYTitle("Event Rate");
  gc2->Modified();
    

  gc2->Print(doMultipliers ? "nu-2d-CrossSection-NT-model-Emu-multiplier.png" : "nu-2d-CrossSection-NT-model-Emu.png");
