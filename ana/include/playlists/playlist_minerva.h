...
namespace NUKECC_ANA{
  ...
  void get_mc_files( TChain* tree , string playlist){
    ifstream f;
    string mybase = getenv("MY_NSFNUKECC");
    if(playlist == "minervame1A")f.open(Form("%s/include/playlists/NukeCC_minervame1A_MC_Inextinguishable_merged.txt",mybase.c_str()));
