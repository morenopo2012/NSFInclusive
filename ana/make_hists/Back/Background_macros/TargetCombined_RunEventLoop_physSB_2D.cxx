...
int main(int argc, char *argv[]){
  ...
  cout<<"Print my name = "<<endl; 
   ...
   if(RunCodeWithSystematics){
       histFileName = utils->GetHistFileName( "PhysicsSideband", FileType::kAny, targetID, targetZ, helicity ); 
     }
   else{
       histFileName = utils->GetHistFileName( "PhysicsBackgd_without", FileType::kAny, targetID, targetZ, helicity );
     } 
   ...
   for (auto v : variablesMC){
	...
	v->m_hists_trans_mc_CH_DS.SyncCVHistos();
     ...
     for (auto v : variables2DMC){
           ...
           v->m_selected_mc_contin_in_trans.SyncCVHistos();
           v->m_selected_mc_trans_in_trans.SyncCVHistos();
           v->m_selected_mc_signal_in_trans.SyncCVHistos();
           v->m_selected_true_signal.SyncCVHistos();
           v->m_selected_trans_CH_US.SyncCVHistos();
           v->m_selected_trans_CH_DS.SyncCVHistos();
           v->m_selected_contin_CH_US.SyncCVHistos();
           v->m_selected_contin_CH_DS.SyncCVHistos();
        }
...
void FillVariable( PlotUtils::ChainWrapper* chain, HelicityType::t_HelicityType helicity, NukeCCUtilsNSF *utils , NukeCC_Cuts *cutter ,NukeCC_Binning  *binsDef ,std::vector<Var*>& variables,std::vector<Var2D*>& variables2d,bool isMC,int targetID, int targetZ, const string playlist, bool doDIS){
  ...
  std::vector<double> planeDNNbin;
   ...
   for(int i=0; i<chain->GetEntries(); ++i){
	 ...
	 reco2++;
	 if(targetIDs[target]<10 && universe->GetInt("NukeCC_targetID") != targetIDs[target]) continue;
         reco3++;
	 ...
	 reco4++;
         ...
         Channels::idx::t_ChannelIdx chanIdx = cutter->GetChannel(universe, helicity);
             ...
             const bool isTrueDIS  = cutter->PassTrueDISCut(universe) && chanIdx == 6;
                
             const bool isTrueinTrans  = (trueW < 2.0 )  && (chanIdx == 1 || chanIdx == 2 || chanIdx == 3 || chanIdx ==4) ; 
             const bool isTrueinContin = (trueW > 2.0 && trueQ2 < 1.0) && chanIdx == 5;
...
//-------------------------------------------------------------Physics Sideband stuff---------------------
               ...
               if( !(cutter->IsInTrueMaterial(universe,targetIDs[target],targetZ,false)) && targetIDs[target]<10){
                 ...
                 if( region == Regions::idx::US && universe->Var("planeDNN" ) > universe->GetTargetPlane(targetIDs[target])-1 && universe->Var("planeDNN" ) < universe->GetTargetPlane(targetIDs[target])+2 ){     
                 ...
                 if( region == Regions::idx::DS && universe->Var("planeDNN" ) > universe->GetTargetPlane(targetIDs[target])-1 && universe->Var("planeDNN" ) < universe->GetTargetPlane(targetIDs[target])+2){    
                        if( isInTrans) {  dst++;
...
//-------------------------------------------------------------2D Physics Sideband stuff---------------------
                if(isInTrans){
                    t1++;
                   if(isTrueinContin){
                      //v->m_selected_mc_trans.GetComponentHist("True_Contin")->Fill(v->GetRecoValue(*universe, 0), wgt);
                      v->m_selected_mc_contin_in_trans.univHist(universe)->Fill(v->GetRecoValueX(*universe, 0), v->GetRecoValueY(*universe, 0), wgt);
                     t3++;
                   }
                    if(isTrueinTrans){
                      //v->m_selected_mc_trans.GetComponentHist("True_Trans")->Fill(v->GetRecoValue(*universe, 0), wgt);
                      v->m_selected_mc_trans_in_trans.univHist(universe)->Fill(v->GetRecoValueX(*universe, 0), v->GetRecoValueY(*universe, 0), wgt);
                     t2++;
                   }
                   if(isTrueDIS){    
                      //v->m_selected_mc_trans.GetComponentHist("True_DIS")->Fill(v->GetRecoValue(*universe, 0), wgt);
                      v->m_selected_mc_signal_in_trans.univHist(universe)->Fill(v->GetRecoValueX(*universe, 0), v->GetRecoValueY(*universe, 0), wgt);
                     t4++;
                   }
               ...
               if(isInContin){
                  c1++;
                   if(isTrueinContin){
                      //v->m_selected_mc_contin.GetComponentHist("True_Contin")->Fill(v->GetRecoValue(*universe, 0), wgt);
                      v->m_selected_mc_contin_in_contin.univHist(universe)->Fill(v->GetRecoValueX(*universe, 0), v->GetRecoValueY(*universe, 0), wgt);
                    c3++;
                   }
                    if(isTrueinTrans){
                     // v->m_selected_mc_contin.GetComponentHist("True_Trans")->Fill(v->GetRecoValue(*universe, 0), wgt);
                      v->m_selected_mc_trans_in_contin.univHist(universe)->Fill(v->GetRecoValueX(*universe, 0), v->GetRecoValueY(*universe, 0), wgt);
                    c2++;
                   }
                   if(isTrueDIS){    
                      //v->m_selected_mc_contin.GetComponentHist("True_DIS")->Fill(v->GetRecoValue(*universe, 0), wgt);
                      v->m_selected_mc_signal_in_contin.univHist(universe)->Fill(v->GetRecoValueX(*universe, 0), v->GetRecoValueY(*universe, 0), wgt);
                    c4++;
                   }
                }
                if(isTrueDIS){
                  s1++;
                      //v->m_selected_mc_sig.GetComponentHist("Signal")->Fill(v->GetRecoValue(*universe, 0), wgt);
                      v->m_selected_true_signal.univHist(universe)->Fill(v->GetRecoValueX(*universe, 0), v->GetRecoValueY(*universe, 0), wgt);
                }
               if( !(cutter->IsInTrueMaterial(universe,targetIDs[target],targetZ,false)) && targetIDs[target]<10){
                    Regions::idx::t_RegionIdx region = cutter->GetRegion(universe, targetIDs[target], targetZ);
                 if( region == Regions::idx::US && universe->Var("planeDNN" ) > universe->GetTargetPlane(targetIDs[target])-1 && universe->Var("planeDNN" ) < universe->GetTargetPlane(targetIDs[target])+2 ){     
                        ...
                        if( isInTrans) { ust++;
                            v->m_selected_trans_CH_US.univHist(universe)->Fill(v->GetRecoValueX(*universe, 0), v->GetRecoValueY(*universe, 0), wgt);
                        }
                        if( isInContin) {      usc++;                                                                  
                            v->m_selected_contin_CH_US.univHist(universe)->Fill(v->GetRecoValueX(*universe, 0), v->GetRecoValueY(*universe, 0), wgt);
                        }
                }

                 if( region == Regions::idx::DS && universe->Var("planeDNN" ) > universe->GetTargetPlane(targetIDs[target])-1 && universe->Var("planeDNN" ) < universe->GetTargetPlane(targetIDs[target])+2){    
                        if( isInTrans) {  dst++;
                            v->m_selected_trans_CH_DS.univHist(universe)->Fill(v->GetRecoValueX(*universe, 0), v->GetRecoValueY(*universe, 0), wgt);
                        }
                        if( isInContin) {  dsc++;                                                                     
                            v->m_selected_contin_CH_DS.univHist(universe)->Fill(v->GetRecoValueX(*universe, 0), v->GetRecoValueY(*universe, 0), wgt);
                        }
                }
            }
...
//------------------------------------------------------Physics 2D scatter plots------------------------------------------------------------------
             if(cutter->IsInTrueMaterial(universe,targetIDs[target],targetZ,false)){
	 ...
	 if( dataverse->GetVecElem("ANN_plane_probs",0) < MIN_PROB_PLANE_CUT ) continue;	   
	   ...
	   for (auto v : variables2d){
         ...
         if(cutter->PassDISCut(dataverse)){data_dis++;}
             if(isintrans){
                  data11++;
                v->m_selected_data_reco_trans.hist->Fill(v->GetRecoValueX(*dataverse), v->GetRecoValueY(*dataverse));
             }
             if(isincontin){   data22++;
                v->m_selected_data_reco_contin.hist->Fill(v->GetRecoValueX(*dataverse), v->GetRecoValueY(*dataverse));
             }
             reco6++;
...
for(auto band : error_bands){
   ...
   std::cout<<" Data 2D trans = "         <<data11<<std::endl;
   std::cout<<" Data 2D contin = "         <<data22<<std::endl;
   std::cout<<"**********************************"<<std::endl;
