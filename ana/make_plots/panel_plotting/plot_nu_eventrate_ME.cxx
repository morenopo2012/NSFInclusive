...
void makePlots(bool doMultipliers, bool doTracks, bool doBkgSub, string location)
{
 // ROOT::Cintex::Cintex::Enable();
  ...
  //CV
  TFile f1(Form("%s_CV/MuonEventSelection_MakeFlux-1_Multiplicity-1_Sample-Signal_CombinedPlaylists-CombinedPlaylists-me1Bme1Cme1Dme1Eme1Fme1Gme1Lme1Mme1Nme1Ome1P.root",location.c_str()));//1track
  TFile f2(Form("%s_CV/MuonEventSelection_MakeFlux-1_Multiplicity-2_Sample-Signal_CombinedPlaylists-CombinedPlaylists-me1Bme1Cme1Dme1Eme1Fme1Gme1Lme1Mme1Nme1Ome1P.root",location.c_str()));//2track
  TFile f3(Form("%s_CV/MuonEventSelection_MakeFlux-1_Multiplicity-0_Sample-Signal_CombinedPlaylists-CombinedPlaylists-me1Bme1Cme1Dme1Eme1Fme1Gme1Lme1Mme1Nme1Ome1P.root",location.c_str()));//Ntrack
  ...
  //Constraint File
  TFile f5(Form("%s_CV/SideBandFit_CombinedPlaylists-CombinedPlaylists-me1Bme1Cme1Dme1Eme1Fme1Gme1Lme1Mme1Nme1Ome1P.root",location.c_str()));
  ...
  mc_qelike_dis->SetLineWidth(2);
  mc_qelike_2p2h->SetLineWidth(2);
  //mc_qelike_2p2h_no_lowrec->SetLineWidth(2);
  ...
  GridCanvas* gc=plotpT1DME(histAndOpts, doMultipliers ? multipliers : NULL);
  // Set the y range manually. Can also use gc->Remax() to guess automatically
  gc->SetYLimits(0, 475);
  //  gc->SetYTitle("Event Rate(x10^{-3}) per GeV^{2}");
  gc->SetYTitle("dN/dp_{t}dp_{||} (#times10^{-3} per GeV^{2})");
  gc->Modified();
  // Example of adding a legend. The co-ordinate system is NDC on the
  // entire canvas, ie (0,0) in the bottom left corner of the canvas
  // (not the individual pad), and (1,1) in the top right
  TLegend* leg=new TLegend(0.15, 0.7, 0.25, 0.9);
  leg->SetFillStyle(0);
  leg->SetBorderSize(0);
  leg->SetTextSize(0.025);
  leg->AddEntry(data, "MINERvA Data", "lpe");
  leg->AddEntry(mc, "MnvGENIE v1", "l");
  if(doTracks){
  ...
  // Values to multiply each bin by to get them on a similar range
  double  multipliers2[]={10, 3, 2, 1, 1,
			  1, 1, 1, 1, 2,
			  3, 10, 100};
  ...
  gc2->SetYLimits(0, 500);
  //  gc2->SetYTitle("Event Rate(x10^{-3}) per GeV^{2}");
  gc2->SetYTitle("dN/dp_{t}dp_{||} (#times10^{-3} per GeV^{2})");
  gc2->Modified();
  if(doTracks){
    if(doBkgSub){
