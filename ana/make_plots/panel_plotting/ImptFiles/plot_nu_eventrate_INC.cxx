...
void makePlots(bool doMultipliers)
{
  ...
  TFile f1("histsEvents/Hists_EventSelection_t5_z82_Nu_v1_.root");//1track
  ...
  double multipliers1[]={1, 2, 3, 8,
                        10, 20, 100, 500,
                        1000, 1000, 10000, 50};
  double multipliers3[]={1, 1, 1, 1,
                        1.5, 2, 1, 1,
                        1, 1, 1, 1};

  GridCanvas* gc=plotXAxis1D(histAndOpts, "Muon Energy (GeV)", "Ehad", doMultipliers ? multipliers1 : NULL);
  //GridCanvas* gc=plotXAxis1D(histAndOpts, "Bjorken Variable x", "y", doMultipliers ? multipliers3 : NULL);
  ...
  // Values to multiply each bin by to get them on a similar range
  double  multipliers4[]={10, 5, 5, 5, 30,
			  50, 100, 300, 1000, 1000,
			  5000, 5000, 10000};

  double  multipliers6[]={1, 1, 1, 1, 1.5,
			  2, 1, 1, 1, 1,
			  1, 1, 1};
  ...
  //GridCanvas* gc2=plotYAxis1D(histAndOpts, "Inelasticity y", "x", doMultipliers ? multipliers6 : NULL);
  gc2->Remax();
