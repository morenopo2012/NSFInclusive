...
TH2* concatenateHists(vector<TH1*>& hists1D, int axis)
{
  ...
  //These are eavail vs pt vs pz
   ...
   int nxBins_pzptrec=10;
   double xBins_pzptrec[nxBins_pzptrec+1]={0,0.020,0.040,0.080,0.120,0.160,0.240,0.320,0.400,0.600,0.7999};
...
vector<std::pair<TH2*, const char*> > getSystHistsAndOpts(MnvH2D* data, bool pt,TLegend *&leg, string group = "",bool isFrac=true)
{
  ...
  for(int i=0; i<nBins; ++i){
    ...
    if(group==""){
      plotter.DrawErrorSummary(proj, "TR", true, true, -1, false,"", isFrac);
...
int makePlots(bool pt, string location, bool drawGroups, string histoname, bool isFrac)
{
  ...
  */
  ...
  for(int i=0;i<2;i++)recoil3Dbins.push_back(i*0.200+0.400);//400,600
  recoil3Dbins.push_back(0.7990);
